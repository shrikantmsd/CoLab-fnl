...
function ProjectsMode() {
  ...
  const [previewIR, setPreviewIR] = useState(null); // {docName, file}
  const [irDocsWidth, setIrDocsWidth] = useState(380); // resizable

  // Persist uploads to localStorage — strip dataUrl (too large for 5MB limit)
  function updateUploads(updater) {
    setUploadedFiles(prev => {
      const next = typeof updater === 'function' ? updater(prev) : updater;
      try {
        // Save metadata only — drop dataUrl which can exceed 5MB
        const lite = {};
        for (const [pid, files] of Object.entries(next)) {
          lite[pid] = {};
          for (const [docName, entry] of Object.entries(files)) {
            lite[pid][docName] = {
              name: entry.name, size: entry.size, type: entry.type,
              dataUrl: entry.dataUrl ? '__uploaded__' : null,
            };
          }
        }
        localStorage.setItem('raisa_india_uploads', JSON.stringify(lite));
    ...
    <div style={{ display:'flex', height:'100%', overflow:'hidden' }}>
      ...
      {proj&&app ? (
        <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
          ...
          {/* Two-pane layout: checklist + preview */}
          <div style={{ flex:1, display:'flex', overflow:'hidden' }}>
          <div style={{ width: irDocsWidth, flexShrink:0, overflowY:'auto', minWidth:0 }}>
            {app.sections.map((sec,si)=>(
              <div key={si}>
                ...
                {sec.docs.map(([type,desc,name],di)=>{
                  ...
                  const isPreviewing = previewIR?.docName === name;
                  return (
                    <div key={di}
                      onClick={() => f && setPreviewIR({ docName: name, file: f })}
                      style={{ display:'flex', alignItems:'center', gap:10, padding:'7px 14px',
                        borderBottom:'1px solid #F3F4F6',
                        background: isPreviewing ? '#DBEAFE' : (f?'#F0FFF4':'transparent'),
                        cursor: f ? 'pointer' : 'default',
                        borderLeft: isPreviewing ? `3px solid ${T.navy}` : '3px solid transparent' }}>
          ...
          <div onMouseDown={e => {
            ...
            const startW = irDocsWidth;
            const onMove = mv => setIrDocsWidth(Math.max(240, Math.min(800, startW + mv.clientX - startX)));
            const onUp = () => { window.removeEventListener('mousemove', onMove); window.removeEventListener('mouseup', onUp); };
            window.addEventListener('mousemove', onMove);
            window.addEventListener('mouseup', onUp);
          }}
          style={{ width:6, flexShrink:0, cursor:'col-resize', background:'#E5E7EB',
            borderLeft:'1px solid #D0D0D0', borderRight:'1px solid #D0D0D0', position:'relative' }}>
            <div style={{ position:'absolute', top:'50%', left:0, width:6, height:50,
              transform:'translateY(-50%)', background:'#9CA3AF', borderRadius:2, opacity:0.6 }}/>
          </div>

          {/* Preview pane */}
          <div style={{ flex:1, minWidth:300, background:'#E8E8E8',
            display:'flex', flexDirection:'column' }}>
            <div style={{ background:cat?.color||T.navy, padding:'8px 14px', display:'flex',
              alignItems:'center', gap:10, flexShrink:0 }}>
              <span style={{ fontSize:12, color:'#fff', fontWeight:700, flex:1,
                whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>
                {previewIR ? `📄 ${previewIR.docName}` : 'Document Preview'}
              </span>
              {previewIR?.file?.dataUrl && previewIR.file.dataUrl !== '__uploaded__' && (
                <a href={previewIR.file.dataUrl} download={previewIR.file.name}
                  style={{ fontSize:10, color:'#fff', padding:'2px 8px',
            ...
            <div style={{ flex:1, overflow:'auto', display:'flex', alignItems:'center',
              ...
              {!previewIR ? (
                <div style={{ textAlign:'center', color:'#9CA3AF' }}>
                  ...
                  <div style={{ fontSize:13, fontWeight:600, marginBottom:6 }}>No document selected</div>
                  <div style={{ fontSize:11 }}>Click any uploaded item on the left to preview</div>
                </div>
              ) : !previewIR.file?.dataUrl || previewIR.file.dataUrl === '__uploaded__' ? (
                <div style={{ textAlign:'center', color:'#9CA3AF', padding:20 }}>
                  <div style={{ fontSize:40, marginBottom:12 }}>📋</div>
                  <div style={{ fontSize:12, fontWeight:600, marginBottom:6, color:'#5D5D5D' }}>
                    {previewIR.file?.name || 'File metadata only'}
                  </div>
                  <div style={{ fontSize:10, lineHeight:1.6 }}>
                    File metadata preserved from previous session.<br/>
                    Re-upload to view inline preview.
                  </div>
                </div>
              ) : previewIR.file.type === 'application/pdf' || (previewIR.file.name||'').toLowerCase().endsWith('.pdf') ? (
                <iframe src={previewIR.file.dataUrl}
                  style={{ width:'100%', height:'100%', border:'none', background:'#fff',
                    boxShadow:'0 2px 12px rgba(0,0,0,0.15)', minHeight:500 }}
                  title={previewIR.file.name}/>
              ) : (previewIR.file.type||'').startsWith('image/') ? (
                <img src={previewIR.file.dataUrl} alt={previewIR.file.name}
                  style={{ maxWidth:'100%', maxHeight:'100%',
                    boxShadow:'0 2px 12px rgba(0,0,0,0.15)', background:'#fff' }}/>
              ) : (
                <div style={{ textAlign:'center', color:'#5D5D5D', padding:20 }}>
                  <div style={{ fontSize:40, marginBottom:12 }}>📎</div>
                  <div style={{ fontSize:12, fontWeight:600, marginBottom:8 }}>{previewIR.file.name}</div>
                  <div style={{ fontSize:10, color:'#9CA3AF', marginBottom:12 }}>
                    {previewIR.file.type || 'Unknown'} · {Math.round((previewIR.file.size||0)/1024)} KB
                  </div>
                  <a href={previewIR.file.dataUrl} download={previewIR.file.name}
                    style={{ fontSize:11, color:'#fff', padding:'6px 14px',
                      background:T.navy, borderRadius:5, textDecoration:'none', display:'inline-block' }}>
                    ⬇ Download File
                  </a>
                </div>
              )}
