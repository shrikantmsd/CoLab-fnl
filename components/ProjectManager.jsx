    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: T.bg,
      ...
      {modal === 'products' && (
        <Modal title="Add Product" onClose={() => setModal(null)}>
          ...
          <Field label="Dosage Form"><Input value={form.dosage_form || ''} onChange={v => setForm(f => ({ ...f, dosage_form: v }))} placeholder="e.g. Film-coated Tablet" /></Field>
      ...
      {modal === 'dossiers' && (
        <Modal title="New Dossier" onClose={() => setModal(null)}>
          ...
          <Field label="Country"><Input value={form.country || ''} onChange={v => setForm(f => ({ ...f, country: v }))} placeholder="e.g. Philippines" /></Field>
          <Field label="Regulatory Authority"><Input value={form.authority || ''} onChange={v => setForm(f => ({ ...f, authority: v }))} placeholder="e.g. FDA-PH, CDSCO, EMA" /></Field>
          <Field label="Submission Type">
            <Select value={form.submission_type || 'NDA'} onChange={v => setForm(f => ({ ...f, submission_type: v }))}
              options={[{value:'NDA',label:'NDA — New Drug Application'},{value:'ANDA',label:'ANDA — Abbreviated NDA'},{value:'MAA',label:'MAA — Marketing Auth.'},{value:'NDS',label:'NDS — New Drug Submission'},{value:'Variation',label:'Variation / Supplement'}]} />
          </Field>
          <Field label="Dossier Format">
            <Select value={form.dossier_format || 'CTD'} onChange={v => setForm(f => ({ ...f, dossier_format: v }))}
              options={[{value:'CTD',label:'ICH CTD'},{value:'ACTD',label:'ASEAN ACTD'}]} />
          </Field>
          <Field label="ICH M4Q Version">
            <Select value={form.m4q_version || 'R1'} onChange={v => setForm(f => ({ ...f, m4q_version: v }))}
              options={[{value:'R1',label:'M4Q(R1) — Current'},{value:'R2',label:'M4Q(R2) — 2027 Draft'}]} />
          </Field>
          <Btn onClick={createItem} style={{ width: '100%', justifyContent: 'center' }}>Create Dossier</Btn>
        </Modal>
      )}
