          ...
          <div style={{ background: '#fff', borderBottom: `1px solid ${T.border}`, flexShrink: 0 }}>
            ...
            <div style={{ padding: '8px 20px', display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <button onClick={exportEctd} disabled={exporting}
                ...
                {exporting ? '⏳ Packaging...' : `⬇ Export SEQ ${activeSeq?.sequence_number || '0000'}`}
              </button>
            </div>
              ...
              <div key={module}>
                ...
                {expandedModules[module] && moduleNodes.map(node => (
                  <NodeRow key={node.id} node={node}
                    ...
                    isNewSequence={activeSeq && activeSeq.sequence_number !== '0000'}
                    onUpload={file => handleUpload(node, file)}
                    onOperationChange={async (nodeId, op) => {
                      await api('/api/dossiers/operation', 'POST', { node_id: nodeId, operation: op });
                      setNodes(prev => prev.map(n => n.id === nodeId ? { ...n, operation: op } : n));
                    }}
