                ...
                const getDisplayLabel = (prefix) => {
                  ...
                  if (neMatch) {
                    ...
                    if (childPath) {
                      ...
                      const childLabel = getLabel(origSection);
                      return childLabel
                        ? `${origSection} — ${childLabel}`
                        : allNodeBySection[prefix]?.title
                          ? `${origSection} — ${allNodeBySection[prefix].title}`
                          : origSection;
                    } else {
                      const parentLabel = getLabel(parentBase) || parentBase;
                      if (meta?.displayName) {
                        return `${parentBase} — ${parentLabel} (${meta.displayName})`;
                      }
                      return `${parentBase} — ${parentLabel} [${neId}]`;
                    }
                  }
                  // ICH metadata on original 3.2.S and 3.2.P
                  if (prefix === '3.2.S' && productINN) {
                    return `3.2.S — Drug Substance (${productINN})`;
                  }
                  if (prefix === '3.2.P' && productINN) {
                    const df = activeDossier?.dosage_form || '';
                    const strength = activeDossier?.strength || '';
                    const dpLabel = df ? `${productINN}, ${df}${strength ? ' ' + strength : ''}` : productINN;
                    return `3.2.P — Drug Product (${dpLabel})`;
                  }
                  const stdLabel = getLabel(prefix);
                  return stdLabel
                    ? `${prefix} — ${stdLabel}`
                    : allNodeBySection[prefix]?.title
                      ? `${prefix} — ${allNodeBySection[prefix].title}`
                      : prefix;
                };
