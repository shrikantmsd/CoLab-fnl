...
class DossierErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, info) { console.error('DossierView crash:', error, info); }
  render() {
    if (this.state.hasError) {
      return React.createElement('div', {
        style: { flex:1, display:'flex', alignItems:'center', justifyContent:'center',
          flexDirection:'column', gap:16, padding:40, fontFamily:'Segoe UI,sans-serif' }
      },
        React.createElement('div', { style:{ fontSize:48 } }, '⚠️'),
        React.createElement('div', { style:{ fontSize:16, fontWeight:700, color:'#1E1E1E' } }, 'Something went wrong'),
        React.createElement('div', { style:{ fontSize:13, color:'#5D5D5D', textAlign:'center', maxWidth:400 } },
          String(this.state.error?.message || 'An error occurred while rendering the dossier view.')),
        React.createElement('button', {
          onClick: () => { this.setState({ hasError:false, error:null }); this.props.onBack?.(); },
          style: { padding:'8px 20px', background:'#2B579A', color:'#fff', border:'none',
...
export default function ProjectManager({ userId = 'user_default' }) {
  ...
  const [treeWidth, setTreeWidth]       = useState(300);  // resizable panel 1
  const [previewWidth, setPreviewWidth] = useState(null); // flex:1 by default
  const [gapWidth, setGapWidth]         = useState(300);  // resizable panel 3
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: T.bg,
      ...
      {view === 'projects' ? (
                  ...
                  {(() => {
                    const filtered = dossierSearch
                      ? dossiers.filter(d =>
                          ...
                          (d.applicant||'').toLowerCase().includes(dossierSearch) ||
                        ...
                        <div style={{ padding: '6px 10px', borderBottom: `1px solid ${T.border}`, flexShrink:0 }}>
                          <input placeholder="🔍 Search dossiers..." value={dossierSearch}
                            ...
                            style={{ width:'100%', padding:'5px 8px', border:`1px solid ${T.border}`,
                              borderRadius:5, fontSize:11, boxSizing:'border-box', fontFamily:'inherit' }}/>
                        </div>
                        {filtered.length === 0 ? (
                          <div style={{ padding: '30px', color: T.muted, fontSize: '13px', textAlign: 'center' }}>
                            {dossierSearch ? `No dossiers match "${dossierSearch}"` : 'No dossiers yet. Create a dossier to start assembling your submission.'}
                          </div>
                        ) : filtered.map(d => (
        ...
        // ── DOSSIER VIEW ───────────────────────────────────────────────────
        <DossierErrorBoundary onBack={() => { setView('projects'); setActiveDossier(null); setNodes([]); setSequences([]); setActiveSeq(null); setDossierError(null); }}>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          {dossierError && (
            <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center',
              ...
              <div style={{ fontSize:16, fontWeight:700, color:T.text }}>Something went wrong</div>
              <div style={{ fontSize:13, color:T.muted, textAlign:'center', maxWidth:400 }}>
                {dossierError}
              </div>
              <div style={{ display:'flex', gap:10 }}>
                <button onClick={() => { setView('projects'); setActiveDossier(null); setNodes([]);
                  setSequences([]); setActiveSeq(null); setDossierError(null); }}
                  style={{ padding:'8px 20px', background:T.accent, color:'#fff', border:'none',
                    borderRadius:6, fontSize:13, fontWeight:600, cursor:'pointer' }}>
                  ← Back to Projects
                </button>
                <button onClick={() => activeDossier && loadDossier(activeDossier)}
          ...
          <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
            ...
            {/* ── PANEL 1: docuBridge-style Dossier Tree ── */}
            <div style={{ width: treeWidth, flexShrink: 0, overflow: 'auto', borderRight: `1px solid ${T.border}`,
              background: '#F5F6F8', display: 'flex', flexDirection: 'column' }}>

              {/* Outline header — docuBridge navy style */}
              <div style={{ background: '#3D5A8A', color: '#fff', padding: '6px 10px',
                ...
                <span style={{ fontSize: 12, fontWeight: 700, flex: 1, fontFamily: "'Segoe UI', sans-serif" }}>
                  Current Outline
                </span>
              ...
              <div style={{ background: '#EEF1F7', borderBottom: '1px solid #D8DBE2',
                ...
                <div style={{ fontWeight: 700, color: '#1A3D6B', overflow: 'hidden',
                  textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {activeDossier?.product_name || '[Product]'}
                </div>
                <div style={{ color: '#888', fontSize: 9 }}>
                  {activeDossier?.country} · {activeDossier?.authority} · {activeDossier?.submission_type}
                </div>
              </div>
              ...
              {nodes.length > 0 && (() => {
                ...
                return (
                  <div style={{ background: '#EAEDF2', borderBottom: '1px solid #D0D0D0',
                    ...
                    <span style={{ fontSize: 9, fontFamily: 'monospace', fontWeight: 700,
                      color: pct === 100 ? '#22C55E' : '#2B579A', minWidth: 32 }}>
                      {pct}%
                    </span>
              ...
              {/* Tree content */}
              {loading ? (
                <div style={{ padding: '40px', textAlign: 'center', color: '#9CA3AF' }}>Loading dossier...</div>
              ) : (() => {
                const LINE = '#CED3DC';
                const seqBadge = (activeSeq?.sequence_number || '0000').padStart(4,'0');

                // Group nodes into a section hierarchy
                // CTD canonical section labels
                const CTD_MODULE_NAMES = {
                  '1':'Module 1 — Administrative / Regional','2':'Module 2 — CTD Summaries',
                  '3':'Module 3 — Quality (CMC)','4':'Module 4 — Nonclinical Study Reports',
                  '5':'Module 5 — Clinical Study Reports',
                };
                const CTD_SEC = {
                  '1.1':'Table of Contents','1.2':'Application Forms','1.3':'Product Information',
                  '1.4':'Expert Information','1.5':'Regional Requirements','1.6':'Environmental Risk Assessment',
                  '2.3':'Quality Overall Summary (QOS)','2.4':'Nonclinical Overview',
                  '2.5':'Clinical Overview','2.6':'Nonclinical Written Summaries','2.7':'Clinical Summary',
                  '3.1':'Table of Contents','3.2':'Body of Data',
                  '3.2.S':'Drug Substance','3.2.S.1':'General Information',
                  '3.2.S.2':'Manufacture','3.2.S.3':'Characterisation',
                  '3.2.S.4':'Control of Drug Substance','3.2.S.5':'Reference Standards',
                  '3.2.S.6':'Container Closure System','3.2.S.7':'Stability',
                  '3.2.P':'Drug Product','3.2.P.1':'Description & Composition',
                  '3.2.P.2':'Pharmaceutical Development','3.2.P.3':'Manufacture',
                  '3.2.P.4':'Control of Excipients','3.2.P.5':'Control of Drug Product',
                  '3.2.P.6':'Reference Standards','3.2.P.7':'Container Closure System',
                  '3.2.P.8':'Stability','3.2.A':'Appendices','3.2.R':'Regional Information',
                  '3.3':'Literature References','4.1':'Table of Contents',
                  '4.2':'Study Reports','4.2.1':'Pharmacology','4.2.2':'Pharmacokinetics',
                  '4.2.3':'Toxicology','5.2':'Tabular Listing','5.3':'Clinical Study Reports',
                  '5.3.1':'Biopharmaceutics','5.3.3':'PK/PD Studies','5.3.5':'Efficacy & Safety',
                };
                const ctdNodeLabel = (node) =>
                  CTD_SEC[node.section]
                    ? `${node.section} — ${CTD_SEC[node.section]}`
                    : node.title ? `${node.section} — ${node.title}` : node.section;

                return Object.entries(nodesByModule).map(([module, moduleNodes]) => {
                  const modKey = 'mod-' + module;
                  const isModOpen = expandedModules[modKey] !== false; // default open
                  const uploaded = moduleNodes.filter(n => n.status !== 'missing').length;

                  // Get top-level sections within this module (e.g. 3.2.S, 3.2.P, 3.2.A)
                  const getTopSec = (section) => {
                    const parts = section.split('.');
                    if (parts.length <= 2) return section;
                    // For Module 3: group by 3.2.S, 3.2.P, 3.2.A etc.
                    if (module.includes('3')) return parts.slice(0, 3).join('.');
                    return parts.slice(0, 2).join('.');
                  };

                  const topSections = [...new Set(moduleNodes.map(n => getTopSec(n.section)))]
                    .sort((a, b) => {
                      const k = s => s.split('.').map(p => isNaN(p) ? p : p.padStart(4,'0')).join('.');
                      return k(a).localeCompare(k(b));
                    });

                  // Section group labels
                  const SEC_LABELS = {
                    '3.2.S':'Drug Substance', '3.2.P':'Drug Product', '3.2.A':'Appendices', '3.2.R':'Regional',
                    '3.2':'Body of Data', '2.3':'Quality Overall Summary', '2.4':'Nonclinical Overview',
                    '2.5':'Clinical Overview', '2.6':'Nonclinical Written Summaries', '2.7':'Clinical Summary',
                    '4.2':'Study Reports', '4.3':'Literature References',
                    '5.2':'Tabular Listing', '5.3':'Clinical Study Reports',
                  };
                  ...
                  return (
                    <div key={module}>
                      {/* Module header row */}
                      <div
                        onClick={() => setExpandedModules(prev => ({ ...prev, [modKey]: !isModOpen }))}
                        style={{ padding: '6px 10px', background: '#2B4A7A', cursor: 'pointer',
                          display: 'flex', alignItems: 'center', gap: 6, position: 'sticky', top: 68, zIndex: 1,
                          borderBottom: '1px solid #1A3D6B' }}>
                        {/* Folder icon */}
                        <svg width="14" height="12" viewBox="0 0 14 12" fill="none" style={{ flexShrink: 0 }}>
                          <path d="M1 2a1 1 0 011-1h3l1.5 1.5H12a1 1 0 011 1v6a1 1 0 01-1 1H2a1 1 0 01-1-1V2z"
                            fill={isModOpen ? '#6B9FD4' : '#4A7AAF'} stroke="#3D6F9F" strokeWidth="0.5"/>
                          {isModOpen && <line x1="1" y1="5" x2="13" y2="5" stroke="#3D6F9F" strokeWidth="0.4"/>}
                        </svg>
                        <span style={{ fontSize: 11, fontWeight: 700, color: '#fff', flex: 1,
                          fontFamily: "'Segoe UI', sans-serif" }}>
                          {isModOpen ? '▼' : '▶'} {CTD_MODULE_NAMES[module] || `Module ${module}`}
                        </span>
                        <span style={{ fontSize: 9, color: 'rgba(255,255,255,0.6)',
                          background: 'rgba(0,0,0,0.2)', padding: '1px 6px', borderRadius: 2 }}>
                          {uploaded}/{moduleNodes.length}
                        </span>
                      </div>

                      {isModOpen && (
                        <div>
                          {topSections.map((topSec, tsIdx) => {
                            const groupNodes = moduleNodes.filter(n => getTopSec(n.section) === topSec);
                            const isGroup = groupNodes.length > 1 || groupNodes[0]?.section !== topSec;
                            const groupKey = 'grp-' + topSec;
                            const isGroupOpen = expandedModules[groupKey] !== false;
                            const isLastGroup = tsIdx === topSections.length - 1;

                            if (isGroup) {
                              // Render as collapsible section group
                              const grpUploaded = groupNodes.filter(n => n.status !== 'missing').length;
                              return (
                                <div key={topSec}>
                                  {/* Section group header */}
                                  <div
                                    onClick={() => setExpandedModules(prev => ({ ...prev, [groupKey]: !isGroupOpen }))}
                                    style={{ display: 'flex', alignItems: 'center', height: 24,
                                      cursor: 'pointer', background: isGroupOpen ? '#E8EDF5' : '#F0F3F8',
                                      borderBottom: '1px solid #D8DCE6', paddingRight: 8 }}
                                    onMouseOver={e => e.currentTarget.style.background = '#DDE3EF'}
                                    onMouseOut={e => e.currentTarget.style.background = isGroupOpen ? '#E8EDF5' : '#F0F3F8'}>

                                    {/* Tree line indent */}
                                    <div style={{ width: 32, flexShrink: 0, display: 'flex',
                                      alignItems: 'stretch', height: '100%' }}>
                                      <div style={{ width: 16, display: 'flex', justifyContent: 'center' }}>
                                        {!isLastGroup && <div style={{ width: 1, background: LINE, height: '100%' }}/>}
                                      </div>
                                      <div style={{ width: 16, position: 'relative' }}>
                                        <div style={{ position: 'absolute', left: 0, top: 0, bottom: '50%', width: 1, background: LINE }}/>
                                        {!isLastGroup && <div style={{ position: 'absolute', left: 0, top: '50%', bottom: 0, width: 1, background: LINE }}/>}
                                        <div style={{ position: 'absolute', left: 0, top: '50%', width: 12, height: 1, background: LINE }}/>
                                      </div>
                                    </div>

                                    {/* Arrow */}
                                    <span style={{ fontSize: 8, color: '#4A6FA5', width: 12, textAlign: 'center', flexShrink: 0 }}>
                                      {isGroupOpen ? '▼' : '▶'}
                                    </span>

                                    {/* Folder icon */}
                                    <svg width="13" height="11" viewBox="0 0 13 11" fill="none" style={{ margin: '0 4px', flexShrink: 0 }}>
                                      <path d="M1 2a1 1 0 011-1h2.5l1 1H11a1 1 0 011 1v5a1 1 0 01-1 1H2a1 1 0 01-1-1V2z"
                                        fill={isGroupOpen ? '#C8D8F0' : '#D8E4F5'} stroke="#8AAFD0" strokeWidth="0.6"/>
                                    </svg>

                                    {/* Label */}
                                    <span style={{ fontSize: 11, fontWeight: 600, color: '#1A3D6B', flex: 1,
                                      fontFamily: "'Segoe UI', sans-serif",
                                      whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                      {CTD_SECTION_LABELS[topSec] ? topSec + ' — ' + CTD_SECTION_LABELS[topSec] : (SEC_LABELS[topSec] ? topSec + ' — ' + SEC_LABELS[topSec] : topSec)}
                                    </span>

                                    <span style={{ fontSize: 9, color: '#6B7280', flexShrink: 0 }}>
                                      {grpUploaded}/{groupNodes.length}
                                    </span>
                                  </div>

                                  {/* Child nodes */}
                                  {isGroupOpen && groupNodes.map((node, ni) => {
                                    const latestDoc = node.documents?.[0];
                                    const isSelected = previewDoc?.node?.id === node.id;
                                    const isLast = ni === groupNodes.length - 1;
                                    return (
                                      <DossierNodeRow key={node.id}
                                        node={node} latestDoc={latestDoc}
                                        isSelected={isSelected} isLast={isLast}
                                        depth={2} seqBadge={seqBadge}
                                        ctdLabel={ctdNodeLabel(node)}
                                        LINE={LINE}
                                        isNewSeq={activeSeq && activeSeq.sequence_number !== '0000'}
                                        onSelect={() => {
                                          if (latestDoc) openPreview(latestDoc, node);
                                          else setPreviewDoc({ node, url: null, filename: null });
                                        }}
                                        onUpload={file => handleUpload(node, file)}
                                        uploading={uploadingNode === node.id}
                                        onOperationChange={async (op) => {
                                          await api('/api/dossiers/operation', 'POST', { node_id: node.id, operation: op });
                                          setNodes(prev => prev.map(n => n.id === node.id ? { ...n, operation: op } : n));
                                        }}
                                      />
                                    );
                                  })}
                              ...
                              // Single node — render as leaf directly
                              ...
                              return (
                                <DossierNodeRow key={node.id}
                                  node={node} latestDoc={latestDoc}
                                  isSelected={isSelected} isLast={tsIdx === topSections.length - 1}
                                  depth={1} seqBadge={seqBadge}
                                  ctdLabel={ctdNodeLabel(node)}
                                  LINE={LINE}
                                  isNewSeq={activeSeq && activeSeq.sequence_number !== '0000'}
                                  onSelect={() => {
                ...
                {!previewDoc ? (
                  ...
                  <div style={{ textAlign: 'center', padding: '40px 20px' }}>
                    ...
                    <div style={{ fontSize: '13px', fontWeight: 600, color: '#374151', marginBottom: 6 }}>
                      {previewDoc.node?.title || 'No document'}
                    </div>
                    <div style={{ fontSize: '11px', color: '#9CA3AF', marginBottom: 16, lineHeight: 1.6 }}>
                    ...
                    <label style={{ cursor: 'pointer' }}>
                      ...
                      <span style={{ padding: '10px 24px', background: '#1A3D6B', color: '#fff',
                        borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: 'pointer',
            ...
            {/* Drag handle — resize between preview and gap panel */}
            <div
              onMouseDown={e => {
                e.preventDefault();
                const startX = e.clientX;
                const startW = gapWidth;
                const onMove = mv => setGapWidth(Math.max(200, Math.min(520, startW - (mv.clientX - startX))));
                const onUp   = () => { window.removeEventListener('mousemove',onMove); window.removeEventListener('mouseup',onUp); };
                window.addEventListener('mousemove',onMove);
                window.addEventListener('mouseup',onUp);
              }}
              style={{ width:5, flexShrink:0, cursor:'col-resize',
                borderLeft:'1px solid #D1D5DB', background:'transparent', position:'relative' }}>
              <div style={{ position:'absolute', top:'50%', left:0, width:5, height:40,
                transform:'translateY(-50%)', background:'#CBD5E1', borderRadius:2, opacity:0.7 }}/>
            </div>

            {/* ── PANEL 3: Node actions + Gap report ── */}
            <div style={{ width: gapWidth, flexShrink:0, overflow: 'auto', background: T.white, display: 'flex', flexDirection: 'column' }}>
