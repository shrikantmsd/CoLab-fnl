...
export default function ProjectManager({ userId = 'user_default' }) {
  ...
  // ── eCTD Export ───────────────────────────────────────────────────────────
  function exportEctd() {
    if (!activeDossier || !nodes.length) return;

    const d = activeDossier;
    const appNum  = (d.authority || 'AUTHORITY').replace(/\s+/g,'-').toUpperCase();
    const product = (d.product_name || 'PRODUCT').replace(/\s+/g,'-').toLowerCase();
    const country = (d.country || 'COUNTRY').replace(/\s+/g,'-').toLowerCase();
    const seq     = '0000';
    const fmt     = d.dossier_format === 'ACTD' ? 'actd' : 'ectd';

    // ── 1. Bash folder-creation script ──────────────────────────────────────
    const folders = new Set();
    nodes.forEach(n => {
      const sec = n.section.toLowerCase().replace(/\./g,'/');
      const mod = n.module.toLowerCase().replace(/\s+/g,'');
      if (fmt === 'ectd') {
        const base = mod.startsWith('module1') ? `m1/${country}`
                   : mod.startsWith('module2') ? 'm2'
                   : mod.startsWith('module3') ? 'm3/32'
                   : mod.startsWith('module4') ? 'm4/42'
                   : 'm5/53';
        folders.add(`${base}/${sec}`);
      } else {
        // ACTD
        const base = mod.startsWith('parti')  ? 'part-i'
                   : mod.startsWith('partii') ? 'part-ii'
                   : mod.startsWith('partiii')? 'part-iii'
                   : 'part-iv';
        folders.add(`${base}/${sec}`);
      }
    });

    const mkdirLines = [...folders].sort().map(f => `mkdir -p "${f}"`).join('\n');

    const bashScript = `#!/bin/bash
# RAISA eCTD Folder Structure Generator
# Dossier : ${d.product_name || 'Product'} — ${d.country}
# Authority: ${d.authority} | Format: ${d.dossier_format} | Version: M4Q(${d.m4q_version})
# Sequence : ${seq} | Generated: ${new Date().toISOString().split('T')[0]}
# ─────────────────────────────────────────────────────────────────────────────

ROOT="${appNum}-${product}-${fmt}-seq${seq}"
mkdir -p "$ROOT"
cd "$ROOT"
...
${d.dossier_format} Dossier — ${d.product_name || 'Product'}
Country    : ${d.country}
Authority  : ${d.authority}
Submission : ${d.submission_type}
Format     : ${d.dossier_format} / M4Q(${d.m4q_version})
Sequence   : ${seq}
Generated  : ${new Date().toISOString().split('T')[0]} by RAISA
READMEEOF

echo "✓ eCTD folder structure created in $ROOT"
echo "  Total sections: ${nodes.length}"
`;

    // ── 2. Node manifest CSV ─────────────────────────────────────────────────
    const csvHeader = 'Module,Section,Title,Required,Status';
    const csvRows = nodes.map(n =>
      `"${n.module}","${n.section}","${n.title.replace(/"/g,'""')}","${n.required ? 'Yes' : 'No'}","${n.status}"`
    );
    const csvContent = [csvHeader, ...csvRows].join('\n');

    // ── 3. index.xml backbone ────────────────────────────────────────────────
    const xmlLeafs = nodes.map(n => {
      const sec = n.section.toLowerCase().replace(/\./g,'-');
      const path = `m${n.module.replace(/[^0-9]/g,'')}/${sec}/${sec}.pdf`;
      return `    <leaf ID="${sec}" xlink:href="${path}" checksum="" checksum-type="md5" operation="new">
      <title>${n.title.replace(/&/g,'&amp;').replace(/</g,'&lt;')}</title>
    </leaf>`;
    }).join('\n');

    const xml = `<?xml version="1.0" encoding="UTF-8"?>
<!-- RAISA eCTD index.xml — ${d.dossier_format} v3.2.2 -->
<!-- Dossier: ${d.product_name} | ${d.country} | ${d.authority} | ${d.submission_type} -->
...
<ich:ich xmlns:ich="http://www.ich.org/ich-ectd-3-2.xsd"
         ...
         version="${seq}">
  <regional>
    <us:us xmlns:us="http://www.fda.gov/ectd">
      <authority>${d.authority}</authority>
      <country>${d.country}</country>
      <submission-type>${d.submission_type}</submission-type>
...
${xmlLeafs}
    ...
    const base = `${appNum}-${product}`;
    setTimeout(() => dl(bashScript,  `${base}-create-folders.sh`,  'text/x-sh'),        0);
    setTimeout(() => dl(csvContent,  `${base}-node-manifest.csv`,  'text/csv'),          400);
    setTimeout(() => dl(xml,         `${base}-index.xml`,           'application/xml'),  800);

    alert(`✓ 3 files downloading:\n\n1. ${base}-create-folders.sh\n   Run this in Terminal to create the eCTD folder structure\n\n2. ${base}-node-manifest.csv\n   Complete list of all ${nodes.length} required sections\n\n3. ${base}-index.xml\n   eCTD backbone XML (LORENZ / EXTEDO compatible)`);
  }
      ...
      {view === 'projects' ? (
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{ background: '#fff', borderBottom: `1px solid ${T.border}`,
            padding: '8px 20px', display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <span style={{ fontSize: '12px', color: T.muted, marginLeft: '10px' }}>
                {activeDossier?.authority} · {activeDossier?.submission_type} · {activeDossier?.dossier_format} M4Q({activeDossier?.m4q_version})
              </span>
            </div>
            <button onClick={exportEctd}
              style={{ background: T.accent, color: '#fff', border: 'none', borderRadius: '6px',
