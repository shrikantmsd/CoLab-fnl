  ...
  return (
    <div>
      ...
      <div style={fieldStyle}>
        ...
        <select style={selectStyle} value={form.product_id || ''} onChange={e => {
          const pid = e.target.value;
          const prod = products.find(p => p.id === pid);
          setForm(f => ({
            ...f,
            product_id: pid,
            inn: prod?.inn || prod?.name || '',
      ...
      {/* INN (API Name) */}
      <div style={fieldStyle}>
        <label style={labelStyle}>API / INN (Drug Substance Name)</label>
        <input style={{...selectStyle, cursor:'text'}} value={form.inn || ''} placeholder="e.g. Paracetamol"
          onChange={e => setForm(f => ({...f, inn: e.target.value}))}/>
      </div>

      {/* API Manufacturer */}
      <div style={fieldStyle}>
        <label style={labelStyle}>API Manufacturer</label>
        <input style={{...selectStyle, cursor:'text'}} value={form.api_manufacturer || ''} placeholder="e.g. Granules India Ltd"
          onChange={e => setForm(f => ({...f, api_manufacturer: e.target.value}))}/>
      </div>
      ...
      <div style={{ display:'flex', gap:10, ...fieldStyle }}>
        <div style={{ flex:1 }}>
          ...
          <input style={{...selectStyle, cursor:'text'}} value={form.dosage_form || ''} placeholder="e.g. Tablet"
            onChange={e => setForm(f => ({...f, dosage_form: e.target.value}))}/>
        </div>
        <div style={{ flex:1 }}>
          <label style={labelStyle}>Strength</label>
          <input style={{...selectStyle, cursor:'text'}} value={form.strength || ''} placeholder="e.g. 500 mg"
...
export default function ProjectManager({ userId = 'user_default' }) {
  ...
  const [nodeExtMeta, setNodeExtMeta] = useState({ name:'', inn:'', manufacturer:'', productName:'', dosageForm:'' });
  const [extMetaMap, setExtMetaMap] = useState({}); // loaded from localStorage per dossier
  ...
  async function loadDossier(dossier) {
    ...
    try {
      ...
      if (result?.error) {
        ...
        // Load hyperlinks for this dossier
        loadHyperlinks(dossier.id);
        // Load extension metadata for display labels
        try {
          const em = JSON.parse(localStorage.getItem(`raisa_ext_meta_${dossier.id}`) || '{}');
          setExtMetaMap(em);
        } catch { setExtMetaMap({}); }
      }
    } catch(err) {
  ...
  async function loadSequences(dossierId) {
    ...
    let seqs = (data || []).sort((a,b) => a.sequence_number.localeCompare(b.sequence_number));

    // Auto-create SEQ 0000 if missing — every dossier MUST have an initial sequence
    const has0000 = seqs.some(s => s.sequence_number === '0000');
    if (!has0000) {
      try {
        const { data: created } = await api('/api/sequences', 'POST', {
        ...
        if (created) seqs = [created, ...seqs];
      } catch(e) { console.warn('Could not auto-create SEQ 0000:', e); }
    }
  ...
  async function handleUpload(node, file) {
    ...
    // Auto-detect cross-references in background
    autoDetectLinks(activeDossier.id, node, file.name);
  }
  ...
  function loadHyperlinks(dossierId) {
    try {
      const saved = JSON.parse(localStorage.getItem(`raisa_links_${dossierId}`) || '[]');
      setHyperlinks(saved);
    } catch { setHyperlinks([]); }
  }

  function saveHyperlinks(dossierId, links) {
    setHyperlinks(links);
    try { localStorage.setItem(`raisa_links_${dossierId}`, JSON.stringify(links)); } catch {}
  }

  function addHyperlink(dossierId, source, target, sourceText, type) {
    const link = {
      id: Date.now().toString(36) + Math.random().toString(36).slice(2,6),
      source_section: source,
      target_section: target,
      source_text: sourceText || `${source} → ${target}`,
      type: type || 'manual',
      created_at: new Date().toISOString(),
    };
    const next = [...hyperlinks, link];
    saveHyperlinks(dossierId, next);
    return link;
  }

  function removeHyperlink(dossierId, linkId) {
    const next = hyperlinks.filter(l => l.id !== linkId);
    saveHyperlinks(dossierId, next);
  }

  // Auto-detect cross-references via AI after upload
  async function autoDetectLinks(dossierId, node, filename) {
    try {
      const res = await fetch('/api/review', {
        ...
        body: JSON.stringify({
          messages: [{ role: 'user', content:
            `You are an eCTD regulatory expert. A document was uploaded to CTD section "${node.section} — ${node.title}" (filename: ${filename}).

Based on common ICH M4Q cross-referencing patterns for this section, list the likely cross-references this document would contain to other CTD sections.

For section ${node.section}, return a JSON array of target sections that are typically referenced. Example:
[{"target":"3.2.S.4.2","reason":"Analytical procedures referenced in specification"},{"target":"3.2.P.5.3","reason":"Validation of analytical procedures"}]

Return ONLY the JSON array, no markdown, no backticks. If no cross-references are typical, return [].` }],
          max_tokens: 500,
      ...
      const refs = JSON.parse(text.replace(/```json|```/g, '').trim());
      if (Array.isArray(refs)) {
        const newLinks = refs
          .filter(r => r.target && r.target !== node.section)
          .map(r => ({
            id: Date.now().toString(36) + Math.random().toString(36).slice(2,6),
            source_section: node.section,
            target_section: r.target,
            source_text: r.reason || `${node.section} → ${r.target}`,
            type: 'auto',
            created_at: new Date().toISOString(),
          }));
        if (newLinks.length > 0) {
          const existing = hyperlinks.map(h => `${h.source_section}-${h.target_section}`);
          const unique = newLinks.filter(l => !existing.includes(`${l.source_section}-${l.target_section}`));
          if (unique.length > 0) {
            const next = [...hyperlinks, ...unique];
            saveHyperlinks(dossierId, next);
          }
        }
      }
    } catch(e) { console.warn('Auto-detect links failed:', e); }
  }

  // ── Node Extension — create sub-folders within a CTD section ──────────
  async function createNodeExtension(parentSection, parentModule) {
    if (!activeDossier?.id) return;
    const meta = nodeExtMeta;

    // Determine display name from metadata based on section type
    const isDS = parentSection === '3.2.S';
    const isDP = parentSection === '3.2.P';
    let displayName = meta.name || '';
    if (isDS) {
      if (!meta.inn?.trim()) { alert('Please enter the API Name (INN)'); return; }
      displayName = `${meta.inn.trim()}${meta.manufacturer?.trim() ? ', ' + meta.manufacturer.trim() : ''}`;
    } else if (isDP) {
      if (!meta.productName?.trim()) { alert('Please enter the Product Name'); return; }
      displayName = `${meta.productName.trim()}${meta.dosageForm?.trim() ? ', ' + meta.dosageForm.trim() : ''}`;
    } else {
      if (!displayName.trim()) { alert('Please enter a name'); return; }
    }

    // Find existing NE extensions
    const directExts = new Set();
    nodes.forEach(n => {
      if (n.section.startsWith(parentSection + '.NE')) {
        const afterParent = n.section.slice(parentSection.length + 1);
        const nePart = afterParent.split('.')[0];
        if (nePart.startsWith('NE')) directExts.add(nePart);
      }
    });
    const nextNum = directExts.size + 1;
    const nePrefix = `${parentSection}.NE${nextNum}`;

    // Find child sections to clone
    const childSections = nodes.filter(n =>
      n.section.startsWith(parentSection + '.') && !n.section.includes('.NE')
    );

    const nodesToCreate = [];
    if (childSections.length > 0) {
      childSections.forEach(child => {
        const relativePath = child.section.slice(parentSection.length);
        nodesToCreate.push({
          dossier_id: activeDossier.id, module: parentModule,
          section: nePrefix + relativePath, title: child.title,
          required: false, status: 'missing',
          sequence_number: activeSeq?.sequence_number || '0000',
        });
      });
    } else {
      nodesToCreate.push({
        dossier_id: activeDossier.id, module: parentModule,
        section: nePrefix, title: displayName,
        required: false, status: 'missing',
        sequence_number: activeSeq?.sequence_number || '0000',
      });
    }
    ...
    try {
      await fetch('/api/dossiers/nodes', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ batch: nodesToCreate, extension_name: displayName, ne_prefix: nePrefix }),
      });

      // Store extension metadata for display labels
      const extMeta = JSON.parse(localStorage.getItem(`raisa_ext_meta_${activeDossier.id}`) || '{}');
      extMeta[nePrefix] = { displayName, inn: meta.inn, manufacturer: meta.manufacturer,
        productName: meta.productName, dosageForm: meta.dosageForm, type: isDS ? 'DS' : isDP ? 'DP' : 'other' };
      localStorage.setItem(`raisa_ext_meta_${activeDossier.id}`, JSON.stringify(extMeta));

      await loadDossierNodes(activeDossier.id, activeSeq?.sequence_number || '0000');
      setNodeExtForm(null);
      setNodeExtMeta({ name:'', inn:'', manufacturer:'', productName:'', dosageForm:'' });
      setExpandedModules(p => ({...p, ['fold-' + nePrefix]: true}));
    } catch(e) {
      console.error('Failed to create node extension:', e);
      alert('Failed to create node extension.');
    }
  }

  // Delete a node extension and all its children
  async function deleteNodeExtension(neSection) {
    if (!activeDossier?.id) return;
    if (!neSection.includes('.NE')) {
      alert('Cannot delete standard CTD sections. Only node extensions can be removed.');
      return;
    }
    const extName = neSection.split('.NE').pop();
    if (!window.confirm(`Delete node extension NE${extName} and all its sub-sections?`)) return;

    try {
      const res = await fetch(`/api/dossiers/nodes?dossier_id=${activeDossier.id}&prefix=${encodeURIComponent(neSection)}`, {
        method: 'DELETE',
      });
      if (!res.ok) {
      ...
      console.error('Failed to delete node extension:', e);
    }
  }
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: T.bg,
          ...
          <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
            ...
            <div style={{ width: treeWidth, flexShrink: 0, overflow: 'auto', borderRight: `1px solid ${T.border}`,
              ...
              {loading ? (
                ...
                try {
                const CTD_MOD = {
                  'Module 1':'1 Administrative Information and Prescribing Information',
                  'Module 2':'2 Common Technical Document Summaries',
                  'Module 3':'3 Quality',
                  'Module 4':'4 Nonclinical Study Reports',
                  'Module 5':'5 Clinical Study Reports',
                };
                const CTD_SEC = {
                  ...
                  '3.2.A':'Appendices','3.2.R':'Regional Information',
                  '3.2.S.1':'General Information','3.2.S.2':'Manufacture',
                  '3.2.S.3':'Characterisation','3.2.S.4':'Control of Drug Substance',
                  '3.2.S.5':'Reference Standards or Materials','3.2.S.6':'Container Closure System',
                  '3.2.S.7':'Stability',
                  // 3.2.P hierarchy
                  '3.2.P.1':'Description and Composition','3.2.P.2':'Pharmaceutical Development',
                  '3.2.P.3':'Manufacture','3.2.P.4':'Control of Excipients',
                  '3.2.P.5':'Control of Drug Product','3.2.P.6':'Reference Standards or Materials',
                  '3.2.P.7':'Container Closure System','3.2.P.8':'Stability',
                  // 3.2.R hierarchy (SAHPRA)
                  '3.2.R.1':'Pharmaceutical and Biological Availability',
                  '3.2.R.2':'Parent API Manufacturer with Various Sites',
                  '3.2.R.3':'Certificate(s) of Suitability (CEPs)',
                  '3.2.R.4':'Multiple API Manufacturers',
                  '3.2.R.5':'Medical Device','3.2.R.6':'Materials of Animal and/or Human Origin',
                  '3.2.R.7':'Batch Records of Samples','3.2.R.8':'Other (SCoRE)',
                  '3.3':'Literature References',
                };
                const SEC_LABELS = {}; // merged into CTD_SEC above

                /* ── Folder row ── */
                const FR = ({ label, depth, isOpen, onClick, count, onDelete }) => (
                  <div onClick={onClick}
                    ...
                    {count != null && <span style={{ fontSize:9, color:'#888', marginLeft:4, marginRight:2 }}>{count}</span>}
                    {onDelete && (
                      <button onClick={e => { e.stopPropagation(); onDelete(); }}
                        style={{ background:'none', border:'none', color:'#CCC', fontSize:11, cursor:'pointer',
                          padding:'0 4px', marginRight:4, lineHeight:1 }}
                        onMouseEnter={e => e.currentTarget.style.color='#EF4444'}
                        onMouseLeave={e => e.currentTarget.style.color='#CCC'}
                        title="Delete node extension">✕</button>
                    )}
                  </div>
                );
                ...
                // Build global section→node map for display labels
                const allNodeBySection = {};
                ...
                const getDisplayLabel = (prefix) => {
                  const neMatch = prefix.match(/^(.+?)\.(NE\d+)(.*)$/);
                  if (neMatch) {
                    const [, parentBase, neId, childPath] = neMatch;
                    const neKey = `${parentBase}.${neId}`;
                    const meta = extMetaMap[neKey];
                    const neMeta = meta?.displayName ? `(${meta.displayName})` : `[${neId}]`;
                    if (childPath) {
                      // Child of NE: show original section + title + NE metadata
                      const origSection = parentBase + childPath;
                      const childLabel = getLabel(origSection) || allNodeBySection[prefix]?.title || '';
                      return childLabel
                        ? `${origSection} — ${childLabel} ${neMeta}`
                        : `${origSection} ${neMeta}`;
                    } else {
                      // NE root: show parent label + NE metadata
                      const parentLabel = getLabel(parentBase) || parentBase;
                      return `${parentBase} — ${parentLabel} ${neMeta}`;
                    }
                  }
                  // Standard sections — propagate ICH metadata to all 3.2.S.* and 3.2.P.* children
                  const stdLabel = getLabel(prefix) || allNodeBySection[prefix]?.title || '';
                  const baseLabel = stdLabel ? `${prefix} — ${stdLabel}` : prefix;

                  if (prefix.startsWith('3.2.S') && dsMeta) return `${baseLabel} ${dsMeta}`;
                  if (prefix.startsWith('3.2.P') && dpMeta) return `${baseLabel} ${dpMeta}`;
                ...
                return (
                  <div>
                    ...
                    {modules.map(mod => {
                      ...
                      /* ── Build trie of section paths ── */
                      const allPrefixes = new Set();
                      const nodeBySection = {};
                      mn.forEach(n => {
                        if (!n.section) return;
                        nodeBySection[n.section] = n;
                        const parts = n.section.split('.');
                        for (let i = 1; i <= parts.length; i++) {
                          allPrefixes.add(parts.slice(0, i).join('.'));
                        }
                      });

                      const ECTD_ORDER = {'S':'01','P':'02','A':'03','R':'04'};
                      const sortKey = s => (s||'').split('.').map(x => {
                        if (ECTD_ORDER[x]) return ECTD_ORDER[x];
                        if (x.startsWith('NE')) return '99' + x;
                        return isNaN(x) ? '50' + x : x.padStart(4,'0');
                      }).join('.');
                      const sortFn = (a,b) => sortKey(a).localeCompare(sortKey(b));

                      const childrenOf = (prefix) => {
                        const wanted = prefix.split('.').length + 1;
                        let children = [...allPrefixes].filter(p => {
                          const pParts = p.split('.');
                          return pParts.length === wanted && p.startsWith(prefix + '.');
                        });

                        // Promote 3.2.S.NE* and 3.2.P.NE* to be children of 3.2 (siblings, not nested)
                        if (prefix === '3.2') {
                          const nePromoted = [...allPrefixes].filter(p =>
                            /^3\.2\.[SP]\.NE\d+$/.test(p)
                          );
                          children = [...children, ...nePromoted];
                        }

                        // Remove NE from 3.2.S and 3.2.P children (they're promoted to 3.2 level)
                        if (prefix === '3.2.S' || prefix === '3.2.P') {
                          children = children.filter(c => !c.includes('.NE'));
                        }

                        return [...new Set(children)].sort(sortFn);
                      };

                      /* Find minimum depth in this module to determine top-level prefixes */
                      let minDepth = Infinity;
                      allPrefixes.forEach(p => { minDepth = Math.min(minDepth, p.split('.').length); });
                      const topPrefixes = [...allPrefixes].filter(p => p.split('.').length === minDepth).sort(sortFn);

                      /* ── Recursive renderer ── */
                      const renderPrefix = (prefix, depth) => {
                        const node = nodeBySection[prefix];
                        const children = childrenOf(prefix);
                        const label = getDisplayLabel(prefix);

                        if (children.length > 0) {
                          /* Folder with sub-folders — virtual or real parent */
                          const fk = 'fold-' + prefix;
                          const fo = expandedModules[fk] !== false;
                          /* Count direct + descendant nodes */
                          const descendants = mn.filter(n => n.section === prefix || n.section.startsWith(prefix + '.'));
                          const fu = descendants.filter(n => n.status !== 'missing').length;
                          const isNE = prefix.includes('.NE');
                          const isDirectNE = isNE && /\.NE\d+$/.test(prefix);
                          return (
                            <div key={prefix}>
                              <FR label={label} depth={depth} isOpen={fo}
                                onClick={() => setExpandedModules(p => ({...p,[fk]:!fo}))}
                                count={`${fu}/${descendants.length}`}
                                onDelete={isDirectNE ? () => deleteNodeExtension(prefix) : undefined}/>
                              {fo && children.map(c => renderPrefix(c, depth + 1))}
                              {/* If this prefix is also a real node with docs, show them */}
                              {fo && node && (node.documents || []).length > 0 &&
                                node.documents.map(d => <DR key={d.id} doc={d} node={node} depth={depth}/>)}
                              {/* + Node Extension button — only at 3.2.S and 3.2.P */}
                              {fo && (prefix === '3.2.S' || prefix === '3.2.P') && (
                                nodeExtForm?.parentSection === prefix ? (
                                  <div style={{ paddingLeft: (depth+1) * 18 + 6, padding:'6px', paddingBottom:8 }}>
                                    <div style={{ background:'#F8F4FF', border:'1px solid #D8B4FE', borderRadius:6, padding:'8px 10px' }}>
                                      <div style={{ fontSize:10, fontWeight:700, color:'#7C3AED', marginBottom:6 }}>
                                        {prefix === '3.2.S' ? '＋ Add Drug Substance (API)' : prefix === '3.2.P' ? '＋ Add Drug Product' : '＋ Node Extension'}
                                      </div>
                                      {prefix === '3.2.S' ? (
                                        <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
                                          <input value={nodeExtMeta.inn} placeholder="API Name (INN) *" autoFocus
                                            onChange={e => setNodeExtMeta(p=>({...p, inn:e.target.value}))}
                                            style={{ padding:'4px 6px', border:'1px solid #D8B4FE', borderRadius:3, fontSize:10, fontFamily:'inherit' }}/>
                                          <input value={nodeExtMeta.manufacturer} placeholder="Manufacturer"
                                            onChange={e => setNodeExtMeta(p=>({...p, manufacturer:e.target.value}))}
                                            onKeyDown={e => { if(e.key==='Enter') createNodeExtension(prefix, node?.module || mod); if(e.key==='Escape') { setNodeExtForm(null); setNodeExtMeta({name:'',inn:'',manufacturer:'',productName:'',dosageForm:''}); } }}
                                            style={{ padding:'4px 6px', border:'1px solid #D8B4FE', borderRadius:3, fontSize:10, fontFamily:'inherit' }}/>
                                        </div>
                                      ) : prefix === '3.2.P' ? (
                                        <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
                                          <input value={nodeExtMeta.productName} placeholder="Product Name *" autoFocus
                                            onChange={e => setNodeExtMeta(p=>({...p, productName:e.target.value}))}
                                            style={{ padding:'4px 6px', border:'1px solid #D8B4FE', borderRadius:3, fontSize:10, fontFamily:'inherit' }}/>
                                          <input value={nodeExtMeta.dosageForm} placeholder="Dosage Form with strength (e.g. Tablet 500 mg)"
                                            onChange={e => setNodeExtMeta(p=>({...p, dosageForm:e.target.value}))}
                                            style={{ padding:'4px 6px', border:'1px solid #D8B4FE', borderRadius:3, fontSize:10, fontFamily:'inherit' }}/>
                                          <input value={nodeExtMeta.manufacturer} placeholder="Manufacturer"
                                            onChange={e => setNodeExtMeta(p=>({...p, manufacturer:e.target.value}))}
                                            onKeyDown={e => { if(e.key==='Enter') createNodeExtension(prefix, node?.module || mod); if(e.key==='Escape') { setNodeExtForm(null); setNodeExtMeta({name:'',inn:'',manufacturer:'',productName:'',dosageForm:''}); } }}
                                        ...
                                        <input value={nodeExtMeta.name} placeholder="Extension name..." autoFocus
                                          onChange={e => setNodeExtMeta(p=>({...p, name:e.target.value}))}
                                          onKeyDown={e => { if(e.key==='Enter') createNodeExtension(prefix, node?.module || mod); if(e.key==='Escape') { setNodeExtForm(null); setNodeExtMeta({name:'',inn:'',manufacturer:'',productName:'',dosageForm:''}); } }}
                                          style={{ width:'100%', padding:'4px 6px', border:'1px solid #D8B4FE', borderRadius:3, fontSize:10, fontFamily:'inherit' }}/>
                                      )}
                                      <div style={{ display:'flex', gap:6, marginTop:6 }}>
                                        <button onClick={() => createNodeExtension(prefix, node?.module || mod)}
                                          style={{ padding:'3px 12px', background:'#7C3AED', color:'#fff', border:'none', borderRadius:3, fontSize:9, fontWeight:600, cursor:'pointer' }}>Add</button>
                                        <button onClick={() => { setNodeExtForm(null); setNodeExtMeta({name:'',inn:'',manufacturer:'',productName:'',dosageForm:''}); }}
                                          style={{ padding:'3px 10px', background:'none', border:'1px solid #D0D0D0', borderRadius:3, fontSize:9, cursor:'pointer', color:'#888' }}>Cancel</button>
                                      </div>
                                    </div>
                                  </div>
                                ) : (
                                  <div onClick={() => setNodeExtForm({ parentSection: prefix, parentModule: node?.module || mod })}
                                    style={{ display:'flex', alignItems:'center', gap:4, cursor:'pointer',
                                      paddingLeft: (depth+1) * 18 + 6, height:22, fontSize:10, color:'#7C3AED' }}
                                    onMouseOver={e => e.currentTarget.style.background='#F5F0FF'}
                                    onMouseOut={e => e.currentTarget.style.background='transparent'}>
                                    <span style={{ color:'#C0C0C0', fontSize:10, fontFamily:'monospace' }}>└─</span>
                          ...
                          /* Leaf node — expandable folder containing documents */
                          ...
                          return (
                            <div key={prefix}>
                              <FR label={label} depth={depth} isOpen={no}
                                onClick={() => {
                                ...
                                onDelete={isDirectNE ? () => deleteNodeExtension(prefix) : undefined}/>
                              {no && (docs.length > 0
                                ? docs.map(d => <DR key={d.id} doc={d} node={node} depth={depth}/>)
                                : <UR node={node} depth={depth}/>
                              )}
                              {/* + Node Extension for leaf nodes — only at 3.2.S and 3.2.P */}
                              {no && (prefix === '3.2.S' || prefix === '3.2.P') && (
                                nodeExtForm?.parentSection === prefix ? (
                                  <div style={{ paddingLeft: (depth+1) * 18 + 6, padding:'6px', paddingBottom:8 }}>
                                    <div style={{ background:'#F8F4FF', border:'1px solid #D8B4FE', borderRadius:6, padding:'8px 10px' }}>
                                      <div style={{ fontSize:10, fontWeight:700, color:'#7C3AED', marginBottom:6 }}>
                                      ...
                                      {prefix === '3.2.S' ? (
                                        <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
                                          ...
                                          <input value={nodeExtMeta.manufacturer} placeholder="Manufacturer"
                                            onChange={e => setNodeExtMeta(p=>({...p, manufacturer:e.target.value}))}
                                            onKeyDown={e => { if(e.key==='Enter') createNodeExtension(prefix, node.module); if(e.key==='Escape') { setNodeExtForm(null); setNodeExtMeta({name:'',inn:'',manufacturer:'',productName:'',dosageForm:''}); } }}
                                            style={{ padding:'4px 6px', border:'1px solid #D8B4FE', borderRadius:3, fontSize:10, fontFamily:'inherit' }}/>
                                        </div>
                                      ) : prefix === '3.2.P' ? (
                                        <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
                                          <input value={nodeExtMeta.productName} placeholder="Product Name *" autoFocus
                                            onChange={e => setNodeExtMeta(p=>({...p, productName:e.target.value}))}
                                            style={{ padding:'4px 6px', border:'1px solid #D8B4FE', borderRadius:3, fontSize:10, fontFamily:'inherit' }}/>
                                          <input value={nodeExtMeta.dosageForm} placeholder="Dosage Form (e.g. Tablets)"
                                            onChange={e => setNodeExtMeta(p=>({...p, dosageForm:e.target.value}))}
                                            style={{ padding:'4px 6px', border:'1px solid #D8B4FE', borderRadius:3, fontSize:10, fontFamily:'inherit' }}/>
                                          <input value={nodeExtMeta.manufacturer} placeholder="Manufacturer"
                                            onChange={e => setNodeExtMeta(p=>({...p, manufacturer:e.target.value}))}
                                      ...
                                      <div style={{ display:'flex', gap:6, marginTop:6 }}>
                                        <button onClick={() => createNodeExtension(prefix, node.module)}
                                          style={{ padding:'3px 12px', background:'#7C3AED', color:'#fff', border:'none', borderRadius:3, fontSize:9, fontWeight:600, cursor:'pointer' }}>Add</button>
                                        <button onClick={() => { setNodeExtForm(null); setNodeExtMeta({name:'',inn:'',manufacturer:'',productName:'',dosageForm:''}); }}
                                          style={{ padding:'3px 10px', background:'none', border:'1px solid #D0D0D0', borderRadius:3, fontSize:9, cursor:'pointer', color:'#888' }}>Cancel</button>
                                      </div>
                                  ...
                                  <div onClick={() => setNodeExtForm({ parentSection: prefix, parentModule: node.module })}
                                    style={{ display:'flex', alignItems:'center', gap:4, cursor:'pointer',
                                      paddingLeft: (depth+1) * 18 + 6, height:22, fontSize:10, color:'#7C3AED' }}
                                    onMouseOver={e => e.currentTarget.style.background='#F5F0FF'}
                                    onMouseOut={e => e.currentTarget.style.background='transparent'}>
                      ...
                      /* Skip redundant module-number folder (e.g. "1" under "1 Administrative...")
                         The module header already represents this level */
                      const modNum = mod.replace('Module ', '');
                      let startPrefixes = topPrefixes;
                      if (topPrefixes.length === 1 && topPrefixes[0] === modNum) {
                        startPrefixes = childrenOf(modNum);
                      }

                      return (
                        <div key={mod}>
                          <FR label={CTD_MOD[mod] || mod} depth={0} isOpen={mo}
                            onClick={() => setExpandedModules(p => ({...p,[mk]:!mo}))}
                            count={`${mu}/${mn.length}`}/>
                          {mo && startPrefixes.map(p => renderPrefix(p, 1))}
                        </div>
                      );
                    })}
                  </div>
      ...
      {/* Hyperlinks Modal */}
      {showLinksModal && (
        <Modal title="🔗 Document Hyperlinks" onClose={() => { setShowLinksModal(false); setAddLinkForm(null); }} width={640}>
          <div style={{ fontSize:12, color:T.muted, marginBottom:12, lineHeight:1.6 }}>
            Cross-references between documents in this dossier. Auto-generated on upload,
            or add manually. These links will be embedded in the eCTD export package.
          </div>

          {/* Add link form */}
          {addLinkForm ? (
            <div style={{ background:'#F0F4F8', borderRadius:8, padding:14, marginBottom:14,
              border:`1px solid ${T.border}` }}>
              <div style={{ fontSize:11, fontWeight:700, marginBottom:8, color:T.accent }}>+ Add Manual Link</div>
              <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
                <select value={addLinkForm.source_section || ''}
                  onChange={e => setAddLinkForm(p => ({...p, source_section: e.target.value}))}
                  style={{ flex:1, padding:'6px 8px', border:`1px solid ${T.border}`,
                    borderRadius:4, fontSize:11, fontFamily:'inherit', minWidth:140 }}>
                  <option value="">Source section...</option>
                  {nodes.map(n => <option key={n.id} value={n.section}>{n.section} — {n.title}</option>)}
                </select>
                <span style={{ fontSize:16, color:T.accent }}>→</span>
                <select value={addLinkForm.target_section || ''}
                  onChange={e => setAddLinkForm(p => ({...p, target_section: e.target.value}))}
                  style={{ flex:1, padding:'6px 8px', border:`1px solid ${T.border}`,
                    borderRadius:4, fontSize:11, fontFamily:'inherit', minWidth:140 }}>
              ...
              <div style={{ display:'flex', gap:8, marginTop:8 }}>
                <button onClick={() => {
                  if (addLinkForm.source_section && addLinkForm.target_section && activeDossier?.id) {
                    addHyperlink(activeDossier.id, addLinkForm.source_section, addLinkForm.target_section,
                      `Manual: ${addLinkForm.source_section} → ${addLinkForm.target_section}`, 'manual');
                    setAddLinkForm(null);
                  }
                }}
                style={{ padding:'6px 16px', background:T.accent, color:'#fff', border:'none',
                  borderRadius:5, fontSize:11, fontWeight:600, cursor:'pointer' }}>
                  Add Link
                </button>
                <button onClick={() => setAddLinkForm(null)}
                  style={{ padding:'6px 16px', background:T.white, color:T.text,
                    border:`1px solid ${T.border}`, borderRadius:5, fontSize:11, cursor:'pointer' }}>
                  Cancel
                </button>
              </div>
            </div>
          ) : (
            <button onClick={() => setAddLinkForm({ source_section:'', target_section:'' })}
              style={{ padding:'6px 16px', background:T.accent, color:'#fff', border:'none',
                borderRadius:5, fontSize:11, fontWeight:600, cursor:'pointer', marginBottom:14 }}>
              + Add Manual Link
            </button>
          )}
          ...
          {hyperlinks.length === 0 ? (
            <div style={{ padding:20, textAlign:'center', color:T.muted, fontSize:12 }}>
              ...
              No hyperlinks yet. Upload documents to auto-detect cross-references,
            ...
            <div style={{ maxHeight:400, overflowY:'auto' }}>
              <div style={{ display:'flex', alignItems:'center', marginBottom:8, gap:8 }}>
                <span style={{ fontSize:10, color:T.muted }}>
                  {hyperlinks.filter(h=>h.type==='auto').length} auto · {hyperlinks.filter(h=>h.type==='manual').length} manual
                </span>
              </div>
              {hyperlinks.map(link => (
                <div key={link.id} style={{ display:'flex', alignItems:'center', gap:8,
                  padding:'8px 12px', borderBottom:'1px solid #F0F0F0', fontSize:11 }}>
                  <span style={{ fontSize:9, padding:'2px 6px', borderRadius:3, fontWeight:700,
                    background: link.type === 'auto' ? '#DBEAFE' : '#FEF3C7',
                    color: link.type === 'auto' ? '#1E40AF' : '#92400E' }}>
                    {link.type === 'auto' ? 'AUTO' : 'MANUAL'}
                  </span>
                  <span style={{ fontWeight:600, color:T.accent, fontFamily:'monospace', fontSize:10 }}>
                    {link.source_section}
                  </span>
                  <span style={{ color:T.muted }}>→</span>
                  <span style={{ fontWeight:600, color:T.green, fontFamily:'monospace', fontSize:10 }}>
                    {link.target_section}
                  </span>
                  <span style={{ flex:1, color:T.muted, fontSize:10, overflow:'hidden',
                    textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                    {link.source_text}
                  </span>
                  <button onClick={() => activeDossier?.id && removeHyperlink(activeDossier.id, link.id)}
