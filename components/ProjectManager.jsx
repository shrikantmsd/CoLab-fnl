...
export default function ProjectManager({ userId = 'user_default' }) {
  ...
  const [nodeExtName, setNodeExtName] = useState('');
...
Return ONLY the JSON array, no markdown, no backticks. If no cross-references are typical, return [].` }],
  ...
  async function createNodeExtension(parentSection, parentModule, extensionName) {
    if (!activeDossier?.id || !extensionName.trim()) return;

    // Find existing NE extensions to determine next number
    const existingExts = nodes.filter(n =>
      n.section.startsWith(parentSection + '.NE') ||
      n.section.startsWith(parentSection + '.ne')
    );
    // Count only direct NE children (not their sub-nodes)
    const directExts = new Set();
    existingExts.forEach(n => {
    ...
    // Find all child sections of parentSection (to clone the sub-tree)
    const childSections = nodes.filter(n =>
      n.section.startsWith(parentSection + '.') &&
      !n.section.includes('.NE')  // don't clone other extensions
    );

    // Build the nodes to create: the extension root + cloned children
    const nodesToCreate = [];

    // If parent has children, clone entire sub-tree under the NE prefix
    if (childSections.length > 0) {
      childSections.forEach(child => {
        // Replace the parent prefix with the NE prefix
        // e.g., 3.2.S.1.1 under 3.2.S → 3.2.S.NE1.1.1
        const relativePath = child.section.slice(parentSection.length);
        const newSection = nePrefix + relativePath;
        nodesToCreate.push({
          dossier_id: activeDossier.id,
          module: parentModule,
          section: newSection,
      ...
      // Leaf node — just create single extension
      nodesToCreate.push({
        dossier_id: activeDossier.id,
        module: parentModule,
        section: nePrefix,
        title: extensionName.trim(),
        required: false,
        status: 'missing',
        sequence_number: activeSeq?.sequence_number || '0000',
      });
    ...
    try {
      ...
      const res = await fetch('/api/dossiers/nodes', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          batch: nodesToCreate,
      ...
      if (!res.ok) {
        ...
        console.error('Node extension error:', err);
      }
                    ...
                    {modules.map(mod => {
                      ...
                      const renderPrefix = (prefix, depth) => {
                        ...
                        const label = getLabel(prefix)
                          ? `${prefix} — ${getLabel(prefix)}`
                          : node?.title ? `${prefix} — ${node.title}` : prefix;
                              ...
                              {fo && (
                                nodeExtForm?.parentSection === prefix ? (
                                  <div style={{ display:'flex', alignItems:'center', gap:4,
                                    paddingLeft: (depth+1) * 18 + 6, height:26 }}>
                                    <span style={{ color:'#C0C0C0', fontSize:10, fontFamily:'monospace' }}>└─</span>
                                    <input value={nodeExtName} placeholder="Extension name..."
                                      autoFocus
                                      onChange={e => setNodeExtName(e.target.value)}
                                      onKeyDown={e => { if(e.key==='Enter' && nodeExtName.trim()) createNodeExtension(prefix, node?.module || mod, nodeExtName); if(e.key==='Escape') { setNodeExtForm(null); setNodeExtName(''); } }}
                                      style={{ flex:1, padding:'2px 6px', border:'1px solid #2B579A', borderRadius:3, fontSize:10, fontFamily:'inherit' }}/>
                                    <button onClick={() => { if(nodeExtName.trim()) createNodeExtension(prefix, node?.module || mod, nodeExtName); }}
                                      style={{ padding:'2px 8px', background:'#2B579A', color:'#fff', border:'none', borderRadius:3, fontSize:9, fontWeight:600, cursor:'pointer' }}>Add</button>
                                    <button onClick={() => { setNodeExtForm(null); setNodeExtName(''); }}
                                      style={{ padding:'2px 6px', background:'none', border:'1px solid #D0D0D0', borderRadius:3, fontSize:9, cursor:'pointer', color:'#888' }}>✕</button>
                                  </div>
                                ) : (
                                ...
                                nodeExtForm?.parentSection === prefix ? (
                                  <div style={{ display:'flex', alignItems:'center', gap:4,
                                    ...
                                    <input value={nodeExtName} placeholder="Extension name..."
                                      autoFocus
                                      onChange={e => setNodeExtName(e.target.value)}
                                      onKeyDown={e => { if(e.key==='Enter' && nodeExtName.trim()) createNodeExtension(prefix, node.module, nodeExtName); if(e.key==='Escape') { setNodeExtForm(null); setNodeExtName(''); } }}
                                      style={{ flex:1, padding:'2px 6px', border:'1px solid #7C3AED', borderRadius:3, fontSize:10, fontFamily:'inherit' }}/>
                                    <button onClick={() => { if(nodeExtName.trim()) createNodeExtension(prefix, node.module, nodeExtName); }}
                                      style={{ padding:'2px 8px', background:'#7C3AED', color:'#fff', border:'none', borderRadius:3, fontSize:9, fontWeight:600, cursor:'pointer' }}>Add</button>
                                    <button onClick={() => { setNodeExtForm(null); setNodeExtName(''); }}
                                      style={{ padding:'2px 6px', background:'none', border:'1px solid #D0D0D0', borderRadius:3, fontSize:9, cursor:'pointer', color:'#888' }}>✕</button>
                                  </div>
                                ) : (
