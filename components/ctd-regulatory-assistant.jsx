  ...
  "positiveFindings": [ "strength 1", "strength 2" ]
        ...
        const runReview = async () => {
          ...
          try {
            ...
            // Build company + context string for AI
          const contextStr = [
            companyProfile.name ? `Company: ${companyProfile.name}` : '',
            companyProfile.country ? `Country: ${companyProfile.country}` : '',
            companyProfile.qualityHead ? `Quality Head: ${companyProfile.qualityHead}` : '',
            companyProfile.isoNumber ? `GMP/ISO Cert: ${companyProfile.isoNumber}` : '',
            companyProfile.notes ? `Notes: ${companyProfile.notes}` : '',
            contextSops.length > 0 ? `Company SOPs available: ${contextSops.map(s=>s.name).join(', ')}` : '',
            contextGuidelines.length > 0 ? `Reference guidelines: ${contextGuidelines.map(g=>`${g.name} (${g.authority})`).join(', ')}` : '',
          ].filter(Boolean).join('\n');

          const basePrompt = getReviewPrompt(drDocType, appInfo.country, appInfo.subType);
          const prompt = contextStr
            ? basePrompt + `\n\nCOMPANY CONTEXT:\n${contextStr}\n\nUse this company context to personalise your review.`
            : basePrompt;
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ display:"flex", flex:1, overflow:"hidden" }}>
                ...
                {/* ── CONTEXT PANE: Company + SOPs + Guidelines ── */}
                {showContextPane && (
                <div style={{ width:320, flexShrink:0, borderLeft:`1px solid ${THEME.border}`,
                  overflowY:"auto", background:"#F8FAFD", display:"flex", flexDirection:"column" }}>

                  {/* Pane header */}
                  <div style={{ padding:"10px 14px", background:"#1A3D6B", display:"flex",
                    alignItems:"center", justifyContent:"space-between", flexShrink:0 }}>
                    <div style={{ fontSize:11, fontWeight:700, color:"#fff",
                      letterSpacing:"0.08em", textTransform:"uppercase" }}>
                      Review Context
                    </div>
                    <button onClick={() => setShowContextPane(false)}
                  ...
                  <div style={{ padding:14, display:"flex", flexDirection:"column", gap:16 }}>
                    ...
                    <div>
                      <div style={{ fontSize:10, fontWeight:700, color:"#1A3D6B",
                        textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:8,
                        paddingBottom:4, borderBottom:`1px solid #D1D5DB` }}>
                        🏢 Company Information
                      </div>
                      {[
                        { key:"name",          label:"Company Name",        ph:"e.g. Pharma Ltd." },
                        { key:"address",       label:"Registered Address",  ph:"Street, City, Country" },
                        { key:"country",       label:"Country of Operation",ph:"e.g. India" },
                        { key:"qualityHead",   label:"Quality Head",        ph:"Name & designation" },
                        { key:"regulatoryHead",label:"Regulatory Head",     ph:"Name & designation" },
                        { key:"isoNumber",     label:"ISO / GMP Cert No.",  ph:"e.g. ISO 9001:2015" },
                      ].map(f => (
                        <div key={f.key} style={{ marginBottom:6 }}>
                          ...
                          <input
                            ...
                            onChange={e => setCompanyProfile(p => ({ ...p, [f.key]: e.target.value }))}
                            placeholder={f.ph}
                      ...
                      <div style={{ marginBottom:6 }}>
                        ...
                        <textarea
                          ...
                          onChange={e => setCompanyProfile(p => ({ ...p, notes: e.target.value }))}
                          placeholder="Any context relevant to regulatory reviews..."
                          rows={2}
                          style={{ width:"100%", padding:"5px 8px", border:"1px solid #D1D5DB",
                            borderRadius:5, fontSize:11, resize:"vertical", boxSizing:"border-box",
                            fontFamily:"inherit" }}
                        />
                      </div>
                    </div>
                    ...
                    <div>
                      <div style={{ fontSize:10, fontWeight:700, color:"#1A3D6B",
                        ...
                        <span style={{ fontSize:9, color:"#9CA3AF", fontWeight:400 }}>
                          {contextSops.length} uploaded
                        </span>
                      </div>

                      {/* SOP list */}
                      {contextSops.map(sop => (
                        <div key={sop.id} style={{ display:"flex", alignItems:"center", gap:6,
                          padding:"5px 8px", background:"#fff", borderRadius:5,
                          border:"1px solid #E5E7EB", marginBottom:4 }}>
                          <span style={{ fontSize:13 }}>📄</span>
                          <div style={{ flex:1, minWidth:0 }}>
                            <div style={{ fontSize:11, fontWeight:500, color:"#1F2937",
                              whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>
                              {sop.name}
                            </div>
                          </div>
                          <button onClick={() => setContextSops(p => p.filter(s => s.id !== sop.id))}
                      ...
                      <label style={{ display:"block", cursor:"pointer" }}>
                        ...
                        />
                        <div style={{ padding:"8px", border:"1.5px dashed #93C5FD",
                          borderRadius:6, textAlign:"center", color:"#2563EB",
                    ...
                    <div>
                      <div style={{ fontSize:10, fontWeight:700, color:"#1A3D6B",
                        ...
                        <span>📚 Regulatory Guidelines</span>
                        <span style={{ fontSize:9, color:"#9CA3AF", fontWeight:400 }}>
                          {contextGuidelines.length} uploaded
                        </span>
                      </div>

                      <div style={{ fontSize:9, color:"#9CA3AF", marginBottom:8, lineHeight:1.5 }}>
                        Upload guidelines from any authority (FDA, EMA, PMDA, CDSCO, ICH). These will be used as reference during document review.
                      </div>

                      {/* Guidelines list */}
                      {contextGuidelines.map(gl => (
                        <div key={gl.id} style={{ padding:"6px 8px", background:"#fff",
                          borderRadius:5, border:"1px solid #E5E7EB", marginBottom:4 }}>
                          <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                            <span style={{ fontSize:13 }}>📘</span>
                            <div style={{ flex:1, minWidth:0 }}>
                              <div style={{ fontSize:11, fontWeight:500, color:"#1F2937",
                            ...
                            <button onClick={() => setContextGuidelines(p => p.filter(g => g.id !== gl.id))}
                              style={{ background:"none", border:"none", color:"#9CA3AF",
                                cursor:"pointer", fontSize:14, lineHeight:1, padding:"0 2px" }}>
                              ×
                            </button>
                      ...
                      {/* Upload guideline with authority tag */}
                      <label style={{ display:"block", cursor:"pointer" }}>
                        <input type="file" accept=".pdf,.doc,.docx" multiple style={{ display:"none" }}
                          onChange={async e => {
                            const files = Array.from(e.target.files);
                            for (const file of files) {
                              ...
                              const authority = prompt(`Authority for "${file.name}":\n(e.g. FDA, EMA, PMDA, ICH, CDSCO)`) || "ICH";
                              setContextGuidelines(p => [...p, {
                                id: Date.now() + Math.random(),
                                name: file.name,
                                authority,
                                base64: b64,
                        ...
                        />
                        <div style={{ padding:"8px", border:"1.5px dashed #6EE7B7",
                          borderRadius:6, textAlign:"center", color:"#059669",
                          fontSize:11, fontWeight:500, background:"#ECFDF5" }}
                          onMouseOver={e => e.currentTarget.style.background="#D1FAE5"}
                          onMouseOut={e => e.currentTarget.style.background="#ECFDF5"}>
                          + Upload Guideline (any authority)
                        </div>
                      </label>

                      {/* Quick add common guidelines */}
                      <div style={{ marginTop:8 }}>
                        <div style={{ fontSize:9, color:"#9CA3AF", marginBottom:4 }}>Quick reference tags:</div>
                        <div style={{ display:"flex", flexWrap:"wrap", gap:4 }}>
                          {["ICH Q1A","ICH Q2","ICH Q3A","ICH Q6A","ICH Q8","ICH Q10","ICH Q11","21 CFR 211","EMA/CHMP"].map(tag => (
                            <span key={tag} style={{ padding:"2px 6px", background:"#F3F4F6",
                              border:"1px solid #E5E7EB", borderRadius:3, fontSize:9,
                    ...
                    {/* ── Context Summary ── */}
                    {(contextSops.length > 0 || contextGuidelines.length > 0 || companyProfile.name) && (
                    <div style={{ padding:"8px 10px", background:"#EFF6FF", borderRadius:6,
                      border:"1px solid #BFDBFE", fontSize:10, color:"#1E40AF" }}>
                      <div style={{ fontWeight:700, marginBottom:4 }}>✓ Review context active</div>
                      {companyProfile.name && <div>Company: {companyProfile.name}</div>}
                      {contextSops.length > 0 && <div>{contextSops.length} SOP(s) loaded</div>}
                      {contextGuidelines.length > 0 && <div>{contextGuidelines.length} guideline(s) loaded</div>}
                      <div style={{ marginTop:4, fontSize:9, color:"#3B82F6" }}>
                        AI review will use this context for analysis
                      </div>
                    </div>
                    )}
                ...
                {/* Toggle context pane when hidden */}
                {!showContextPane && (
                  <button onClick={() => setShowContextPane(true)}
                    style={{ position:"absolute", right:0, top:"50%", transform:"translateY(-50%)",
                      background:"#1A3D6B", color:"#fff", border:"none", borderRadius:"6px 0 0 6px",
                      padding:"8px 4px", cursor:"pointer", fontSize:10, writingMode:"vertical-rl",
                      zIndex:10 }}>
                    Context
                  </button>
                )}
