...
export default function App({ initialMode = "checklist" }) {
  ...
  const [landingMode, setLandingMode] = useState(initialMode === "review" ? "review" : initialMode === "checklist" ? "checklist" : "landing");

  // Expose tab setter to parent nav bar
  React.useEffect(() => {
    window.__raisaSetTab = setActiveTab;
    return () => { delete window.__raisaSetTab; };
  }, [setActiveTab]);
  ...
  const [nodeFiles, setNodeFiles]             = useState({});  // { [sectionKey]: {filename, objectUrl, size} }
  const [outlinePreview, setOutlinePreview]   = useState(null); // { url, filename, section }
  ...
  // Auto-open review panel when entering review mode from landing
  React.useEffect(() => {
    if (landingMode === "review") setDocReviewOpen(true);
  }, [landingMode]);

  // ── Company profile + context pane state ──────────────────────────────────
  const [companyProfile, setCompanyProfile] = React.useState({
    name: '', address: '', country: '', website: '', qualityHead: '', regulatoryHead: '',
    gmpCertified: false, isoNumber: '', notes: '',
  });
  const [contextSops, setContextSops] = React.useState([]);     // [{id, name, file, base64}]
  const [contextGuidelines, setContextGuidelines] = React.useState([]); // [{id, name, authority, file, base64}]
  const [showContextPane, setShowContextPane] = React.useState(true);
  ...
  if (landingMode === "landing") {
    return (
      <div style={{ display:"flex", flexDirection:"column", height:"100vh",
        fontFamily:"'Segoe UI', Arial, sans-serif", background:"#F0F4F8",
        alignItems:"center", justifyContent:"center" }}>
        <div style={{ marginBottom:40, textAlign:"center" }}>
          <div style={{ fontSize:11, fontWeight:700, color:"#1A3D6B", letterSpacing:"0.15em",
            textTransform:"uppercase", marginBottom:6 }}>
            RAISA — Regulatory Affairs Intelligence System & Analytics
          </div>
          <div style={{ fontSize:30, fontWeight:800, color:"#1A3D6B", marginBottom:10 }}>
            Checklist & Review
          </div>
          <div style={{ fontSize:14, color:"#6B7280", maxWidth:460, lineHeight:1.7, margin:"0 auto" }}>
            Select a mode to continue. You can switch between them at any time.
          </div>
        </div>
        ...
        <div style={{ display:"flex", gap:32 }}>
          ...
          <div onClick={() => setLandingMode("checklist")}
            style={{ width:280, padding:"36px 28px", background:"#fff",
              border:"2px solid #1A3D6B", borderRadius:16, cursor:"pointer",
              boxShadow:"0 4px 24px rgba(26,61,107,0.10)", transition:"all .2s" }}
            onMouseOver={e => { e.currentTarget.style.background="#1A3D6B"; e.currentTarget.style.transform="translateY(-4px)"; e.currentTarget.style.boxShadow="0 12px 32px rgba(26,61,107,0.22)"; }}
            onMouseOut={e => { e.currentTarget.style.background="#fff"; e.currentTarget.style.transform="translateY(0)"; e.currentTarget.style.boxShadow="0 4px 24px rgba(26,61,107,0.10)"; }}>

            <div style={{ width:56, height:56, borderRadius:14, background:"#EEF4FF",
              display:"flex", alignItems:"center", justifyContent:"center", marginBottom:20 }}>
              <svg width="30" height="30" viewBox="0 0 30 30" fill="none">
                <rect x="3" y="2" width="20" height="26" rx="3" fill="none" stroke="#1A3D6B" strokeWidth="2"/>
                <path d="M8 10h10M8 15h10M8 20h6" stroke="#1A3D6B" strokeWidth="1.8" strokeLinecap="round"/>
                <circle cx="24" cy="24" r="5" fill="#1A3D6B"/>
                <path d="M22 24l1.5 1.5L26 22" stroke="#fff" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </div>

            <div style={{ fontSize:18, fontWeight:700, color:"#1A3D6B", marginBottom:10 }}>
              Regulatory Checklist
            </div>
            <div style={{ fontSize:13, color:"#6B7280", lineHeight:1.7, marginBottom:20 }}>
              Select country, authority, submission type and product category. Get the complete required document checklist per ICH CTD / ACTD — with M4Q(R1) and M4Q(R2) support across 50+ countries.
            </div>
            <div style={{ fontSize:12, fontWeight:700, color:"#1A3D6B", letterSpacing:"0.05em" }}>
              OPEN CHECKLIST →
            </div>
          </div>

          {/* ── AI DOCUMENT REVIEW ── */}
          <div onClick={() => setLandingMode("review")}
            style={{ width:280, padding:"36px 28px", background:"#fff",
              border:"2px solid #C05000", borderRadius:16, cursor:"pointer",
              boxShadow:"0 4px 24px rgba(192,80,0,0.10)", transition:"all .2s" }}
            onMouseOver={e => { e.currentTarget.style.background="#C05000"; e.currentTarget.style.transform="translateY(-4px)"; e.currentTarget.style.boxShadow="0 12px 32px rgba(192,80,0,0.22)"; }}
            onMouseOut={e => { e.currentTarget.style.background="#fff"; e.currentTarget.style.transform="translateY(0)"; e.currentTarget.style.boxShadow="0 4px 24px rgba(192,80,0,0.10)"; }}>

            <div style={{ width:56, height:56, borderRadius:14, background:"#FFF3E0",
              display:"flex", alignItems:"center", justifyContent:"center", marginBottom:20 }}>
              <svg width="30" height="30" viewBox="0 0 30 30" fill="none">
                <rect x="3" y="2" width="18" height="24" rx="3" fill="none" stroke="#C05000" strokeWidth="2"/>
                <path d="M8 9h10M8 14h10M8 19h5" stroke="#C05000" strokeWidth="1.8" strokeLinecap="round"/>
                <circle cx="23" cy="23" r="5.5" fill="#C05000"/>
                <line x1="23" y1="20.5" x2="23" y2="25.5" stroke="#fff" strokeWidth="1.6" strokeLinecap="round"/>
                <line x1="20.5" y1="23" x2="25.5" y2="23" stroke="#fff" strokeWidth="1.6" strokeLinecap="round"/>
              </svg>
            </div>

            <div style={{ fontSize:18, fontWeight:700, color:"#C05000", marginBottom:10 }}>
              AI Document Review
            </div>
            <div style={{ fontSize:13, color:"#6B7280", lineHeight:1.7, marginBottom:20 }}>
              Upload any regulatory document — CTD module, cover letter, study report, SOP. Get an instant AI-powered compliance review with scores, critical issues and ICH guideline references.
            </div>
            <div style={{ fontSize:12, fontWeight:700, color:"#C05000", letterSpacing:"0.05em" }}>
              OPEN REVIEW →
            </div>
          </div>
        ...
        <div style={{ marginTop:52, fontSize:11, color:"#9CA3AF", textAlign:"center" }}>
          CoLAB Pharma Services · RAISA Platform · Regulatory Affairs Intelligence System & Analytics
        </div>
      </div>
    );
  }

  // ── CHECKLIST MODE — show full checklist tool ────────────────────────────
  if (landingMode === "review") {
    // Jump directly to Document Review panel
    if (!docReviewOpen) setDocReviewOpen(true);
  }
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', 'Calibri', Arial, sans-serif", background: THEME.bg, height: "100vh", color: THEME.text, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ background: "#F3F3F3", borderBottom: "1px solid #C8C8C8", display: "flex", alignItems: "stretch", flexShrink: 0, userSelect: "none" }}>
        ...
        {/* ── Back to landing ── */}
        <div style={{ display:"flex", alignItems:"center", borderRight:"1px solid #D8D8D8", padding:"0 6px" }}>
          <button onClick={() => setLandingMode("landing")}
            ...
            title="Back to Checklist & Review home">
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
              <path d="M9 2.5L4.5 7 9 11.5" stroke="#1A3D6B" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
            <span style={{ fontSize:9, color:"#1A3D6B", fontWeight:700 }}>Home</span>
          </button>
          <div style={{ width:1, height:28, background:"#D8D8D8", marginLeft:4 }}/>
        </div>
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        {!outlineCollapsed && (() => {
          ...
          const OTreeNode = ({ section, label, depth, color, isLast, module, allDocs, seqNum }) => {
            const key = "o-" + section;
            ...
            const childSections = [...new Set(
              allDocs.filter((d) => d.section !== section && d.section.startsWith(section + "."))
                .map((d) => { const rest = d.section.slice(section.length + 1); return section + "." + rest.split(".")[0]; })
            )].sort((a, b) => {
              const k = s => s.split(".").map(seg => isNaN(seg) ? seg : seg.padStart(4,"0")).join(".");
              return k(a).localeCompare(k(b));
            });
            const isFolder = childSections.length > 0 || myDocs.length > 0;
            ...
            return (
              <div>
                ...
                <div
                  onClick={() => {
                    setReqDocsCollapsed(p => ({ ...p, [key]: !p[key] }));
                    setSelectedDocNode(section);
                  }}
                  style={{
                    display:"flex", alignItems:"center", height:22, cursor:"pointer",
                    background: isSelected ? "#D6E4F7" : "transparent",
                    borderLeft: isSelected ? "3px solid #2B579A" : "3px solid transparent",
                    userSelect:"none",
                  }}
                  onMouseOver={e => { if(!isSelected) e.currentTarget.style.background="#EEF3FB"; }}
                  onMouseOut={e => { if(!isSelected) e.currentTarget.style.background="transparent"; }}>

                  {/* Tree lines + indent */}
                  <div style={{ width: indentPx + 20, flexShrink:0, display:"flex", alignItems:"stretch", height:"100%" }}>
                    {Array.from({ length: depth }).map((_, i) => (
                      <div key={i} style={{ width:18, display:"flex", justifyContent:"center", flexShrink:0 }}>
                  ...
                  {/* Expand arrow */}
                  <span style={{ fontSize:9, color:"#888", width:12, textAlign:"center", flexShrink:0 }}>
                    {isFolder ? (isCol ? "▶" : "▼") : ""}
                  </span>

                  {/* Folder / Document icon */}
                  {isFolder
                    ? <svg width="14" height="12" viewBox="0 0 14 12" fill="none" style={{ margin:"0 4px", flexShrink:0 }}>
                        <path d="M1 2.5a1 1 0 011-1h3l1 1.5h6a1 1 0 011 1v5a1 1 0 01-1 1H2a1 1 0 01-1-1V2.5z"
                          fill={isCol ? "#B8C4D8" : "#D6E0F0"} stroke="#8899BB" strokeWidth="0.7"/>
                        {!isCol && <line x1="1" y1="5.5" x2="13" y2="5.5" stroke="#8899BB" strokeWidth="0.5"/>}
                      </svg>
                    : <svg width="12" height="14" viewBox="0 0 12 14" fill="none" style={{ margin:"0 4px", flexShrink:0 }}>
                        <rect x="1" y="0.5" width="10" height="13" rx="1.5" fill="#fff" stroke="#A0AABB" strokeWidth="1"/>
                        <line x1="3" y1="4" x2="9" y2="4" stroke="#C0CADC" strokeWidth="0.8"/>
                        <line x1="3" y1="6.5" x2="9" y2="6.5" stroke="#C0CADC" strokeWidth="0.8"/>
                        <line x1="3" y1="9" x2="7" y2="9" stroke="#C0CADC" strokeWidth="0.8"/>
                      </svg>}
                  ...
                  <span style={{
                    ...
                    color: isSelected ? "#1A3D6B" : depth === 0 ? "#1A3D6B" : "#2D3748",
                    whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis",
                    padding:"0 4px 0 0", lineHeight:"22px",
                ...
                {/* ── Document leaves with upload ── */}
                {!isCol && (
                  <div>
                    {myDocs.map((doc, di) => {
                      const docKey = module + "-" + doc.section;
                      const fileEntry = nodeFiles[docKey];
                      const isDocSel = selectedDocNode === docKey;
                      const isLastDoc = di === myDocs.length - 1 && childSections.length === 0;
                      ...
                      return (
                        <div key={docKey}>
                          ...
                          <div
                            onClick={() => {
                              setSelectedDocNode(docKey);
                              if (fileEntry) setOutlinePreview({ url: fileEntry.objectUrl, filename: fileEntry.filename, section: doc.section, title: doc.title });
                            }}
                            style={{
                              display:"flex", alignItems:"center", height:22, cursor:"pointer",
                              background: isDocSel ? "#D0E8FF" : "transparent",
                              borderLeft: isDocSel ? "3px solid #2B579A" : "3px solid transparent",
                            }}
                            onMouseOver={e => { if(!isDocSel) e.currentTarget.style.background="#F0F6FF"; }}
                            onMouseOut={e => { if(!isDocSel) e.currentTarget.style.background="transparent"; }}>

                            {/* Tree lines */}
                            <div style={{ width:(depth+1)*18+20, flexShrink:0, display:"flex", alignItems:"stretch", height:"100%" }}>
                              {Array.from({ length: depth+1 }).map((_, i) => (
                                <div key={i} style={{ width:18, display:"flex", justifyContent:"center" }}>
                                  <div style={{ width:1, background:LINE, height:"100%" }}/>
                                </div>
                              ))}
                              <div style={{ width:20, position:"relative" }}>
                                <div style={{ position:"absolute", left:0, top:0, bottom:"50%", width:1, background:LINE }}/>
                                {!isLastDoc && <div style={{ position:"absolute", left:0, top:"50%", bottom:0, width:1, background:LINE }}/>}
                                <div style={{ position:"absolute", left:0, top:"50%", width:12, height:1, background:LINE }}/>
                              </div>
                            </div>
                            ...
                            {fileEntry
                              ? <svg width="12" height="14" viewBox="0 0 12 14" fill="none" style={{ margin:"0 4px", flexShrink:0 }}>
                                  <rect x="1" y="0.5" width="10" height="13" rx="1.5" fill="#FFF4E6" stroke="#E07800" strokeWidth="1.2"/>
                                  <line x1="3" y1="4" x2="9" y2="4" stroke="#E07800" strokeWidth="0.9"/>
                                  <line x1="3" y1="6.5" x2="9" y2="6.5" stroke="#E07800" strokeWidth="0.9"/>
                                  <line x1="3" y1="9" x2="7" y2="9" stroke="#E07800" strokeWidth="0.9"/>
                                </svg>
                              : <svg width="12" height="14" viewBox="0 0 12 14" fill="none" style={{ margin:"0 4px", flexShrink:0 }}>
                                  <rect x="1" y="0.5" width="10" height="13" rx="1.5" fill="#FAFAFA" stroke="#C0CADC" strokeWidth="0.9"/>
                                  <line x1="3" y1="4" x2="9" y2="4" stroke="#D0D8E8" strokeWidth="0.8"/>
                                  <line x1="3" y1="6.5" x2="9" y2="6.5" stroke="#D0D8E8" strokeWidth="0.8"/>
                                  <line x1="3" y1="9" x2="7" y2="9" stroke="#D0D8E8" strokeWidth="0.8"/>
                                </svg>}
                            ...
                            <div style={{ flex:1, minWidth:0, display:"flex", alignItems:"center", gap:4 }}>
                              ...
                              <span style={{
                                fontSize:11, color: isDocSel ? "#1A3D6B" : fileEntry ? "#1F2937" : "#6B7280",
                                whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis",
                                fontWeight: fileEntry ? 500 : 400,
                                fontStyle: doc.required ? "normal" : "italic",
                                fontFamily:"'Segoe UI', Arial, sans-serif",
                              }}>
                                {fileEntry ? fileEntry.filename : doc.title}
                              </span>
                            </div>

                            {/* Upload button */}
                            <label
                              title={fileEntry ? "Replace document" : "Upload document"}
                              style={{ cursor:"pointer", flexShrink:0, marginRight:4 }}
                              ...
                              <input type="file" accept=".pdf,.doc,.docx" style={{ display:"none" }}
                                onChange={e => {
                                  ...
                                  if (nodeFiles[docKey]?.objectUrl) URL.revokeObjectURL(nodeFiles[docKey].objectUrl);
                                  const objectUrl = URL.createObjectURL(file);
                                  setNodeFiles(p => ({ ...p, [docKey]: { filename: file.name, objectUrl, size: file.size } }));
                                  setOutlinePreview({ url: objectUrl, filename: file.name, section: doc.section, title: doc.title });
                                  setSelectedDocNode(docKey);
                                  e.target.value = "";
                                }}
                              />
                              <span style={{
                                fontSize:9, padding:"1px 5px", borderRadius:2, lineHeight:"16px",
                                background: fileEntry ? "#E8F5E9" : "#EEF3FB",
                                border: `1px solid ${fileEntry ? "#A5D6A7" : "#BFD3EF"}`,
                                color: fileEntry ? "#2E7D32" : "#2B579A",
                    ...
                    {childSections.map((cs, ci) => (
                      <OTreeNode key={cs} section={cs}
                        label={FOLDER_LABELS_O[cs] || cs}
                        depth={depth+1} color={color}
                        isLast={ci === childSections.length - 1}
                        module={module} allDocs={allDocs} seqNum={seqNum}
                      />
                    ))}
                  </div>
                )}
              ...
              <div style={{ flex: 1, overflowY: "auto", padding: "6px 0" }}>
                ...
                {appInfo.appType === "product" && !(appInfo.country === "IN" && appInfo.subType === "CT-IN") && [1,2,3,4,5].map((mn) => {
                  ...
                  const DocLeaf = ({ doc, groupDocs, idx }) => {
                    ...
                    const fileEntry = nodeFiles[dk];
                    const LINE    = "#CED3DC";
                    const seqStr  = (appInfo.sequence||"0000").padStart(4,"0");

                    return (
                      <div
                        onClick={() => {
                          setSelectedDocNode(dk);
                          if (fileEntry) setOutlinePreview({ url: fileEntry.objectUrl, filename: fileEntry.filename, section: doc.section, title: doc.title });
                        }}
                        style={{
                          display:"flex", alignItems:"center", height:22, cursor:"pointer",
                          background: iDS ? "#D0E8FF" : "transparent",
                          borderLeft: iDS ? "3px solid #2B579A" : "3px solid transparent",
                        }}
                        onMouseOver={e => { if(!iDS) e.currentTarget.style.background="#F0F6FF"; }}
                        onMouseOut={e => { if(!iDS) e.currentTarget.style.background="transparent"; }}>

                        {/* Indent + tree lines */}
                        <div style={{ width:56, display:"flex", alignItems:"stretch", height:"100%", flexShrink:0 }}>
                        ...
                        <div style={{ flex:1, minWidth:0, display:"flex", alignItems:"center", gap:4 }}>
                          ...
                          <span style={{
                            fontSize:11, color: iDS ? "#1A3D6B" : fileEntry ? "#1F2937" : "#6B7280",
                            whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis",
                            fontWeight: fileEntry ? 500 : 400,
                            fontStyle: doc.required ? "normal" : "italic",
                            fontFamily:"'Segoe UI', Arial, sans-serif",
                          ...
                          {!doc.required && (
                            <span style={{ fontSize:8, color:"#9CA3AF", flexShrink:0 }}>opt</span>
                          )}
                        </div>

                        {/* Upload button */}
                        <label title={fileEntry ? "Replace document" : "Upload document"}
                          style={{ cursor:"pointer", flexShrink:0, marginRight:4 }}
                          onClick={e => e.stopPropagation()}>
                          <input type="file" accept=".pdf,.doc,.docx" style={{ display:"none" }}
                            onChange={e => {
                              const file = e.target.files[0];
                              if (!file) return;
                              if (nodeFiles[dk]?.objectUrl) URL.revokeObjectURL(nodeFiles[dk].objectUrl);
                              const objectUrl = URL.createObjectURL(file);
                              setNodeFiles(p => ({ ...p, [dk]: { filename: file.name, objectUrl, size: file.size } }));
                              setOutlinePreview({ url: objectUrl, filename: file.name, section: doc.section, title: doc.title });
                              setSelectedDocNode(dk);
                              e.target.value = "";
                            }}
                          />
                          <span style={{
                            fontSize:9, padding:"1px 5px", borderRadius:2, lineHeight:"16px", display:"inline-block",
                            background: fileEntry ? "#E8F5E9" : "#EEF3FB",
                            border: `1px solid ${fileEntry ? "#A5D6A7" : "#BFD3EF"}`,
                            color: fileEntry ? "#2E7D32" : "#2B579A",
                          }}>
                    ...
                    return topSecs.map((ts, ti) => {
                      ...
                      return (
                        <OTreeNode key={ts} section={ts} label={FOLDER_LABELS_O[ts] || ts}
                          depth={1} color={col} isLast={ti === topSecs.length-1} module={mn} allDocs={groupDocs} seqNum={(appInfo.sequence||"0000").padStart(4,"0")} />
                      );
                    });
                  };
        ...
        {selectedDocNode && (() => {
          ...
          const fileEntry = nodeFiles[selectedDocNode];
          // Use outlinePreview url if available (from upload), else null
          const previewUrl = outlinePreview?.url || null;

          return (
            <div style={{ width:380, flexShrink:0, borderLeft:"1px solid #C8CDD6",
              display:"flex", flexDirection:"column", background:"#fff",
              boxShadow:"-2px 0 8px rgba(0,0,0,0.06)", overflow:"hidden" }}>
              ...
              <div style={{ background:"#3D5A8A", color:"#fff", padding:"6px 12px",
                ...
                <span style={{ fontSize:11, fontWeight:700, flex:1,
                  ...
                  [{seqBadge}] {doc ? doc.section + " — " + doc.title : selectedDocNode}
                </span>
                <button onClick={() => { setSelectedDocNode(null); setOutlinePreview(null); }}
                  style={{ background:"rgba(255,255,255,0.15)", border:"1px solid rgba(255,255,255,0.3)",
                    color:"#fff", cursor:"pointer", padding:"1px 7px", borderRadius:2, fontSize:12 }}>✕</button>
              </div>

              {/* Meta strip */}
              {doc && (
                <div style={{ background:"#F2F5FA", borderBottom:"1px solid #D8DBE8",
                  ...
                  <span style={{ fontSize:9, padding:"2px 7px", borderRadius:10, fontWeight:700,
                    ...
                    color: doc.required ? "#2B579A" : "#888",
                    border:"1px solid " + (doc.required ? "#B8CCE4" : "#DDD") }}>
                    {doc.required ? "Required" : "Optional"}
                  </span>
                  <span style={{ fontSize:9, color:"#888" }}>
                    Module {doc.module} · {doc.section}
                  </span>
                  {fileEntry && (
                    <span style={{ fontSize:9, color:"#2E7D32", background:"#E8F5E9",
                      border:"1px solid #A5D6A7", padding:"2px 7px", borderRadius:10, fontWeight:600 }}>
                      ✓ Uploaded
                    </span>
                  )}
                </div>
              ...
              <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden",
                ...
                {previewUrl ? (
                  <>
                    {/* PDF toolbar */}
                    <div style={{ background:"#3D5A8A", padding:"4px 10px",
                      display:"flex", alignItems:"center", gap:8, flexShrink:0 }}>
                      <span style={{ fontSize:10, color:"rgba(255,255,255,0.85)", flex:1,
                        whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>
                        {fileEntry?.filename || "Document"}
                      </span>
                  ...
                  <div style={{ flex:1, display:"flex", flexDirection:"column",
                    ...
                    <svg width="48" height="60" viewBox="0 0 48 60" fill="none">
                      <rect x="2" y="2" width="44" height="56" rx="4" fill="#F5F7FA"
                        stroke="#C8D0E0" strokeWidth="2"/>
                      <path d="M12 22h24M12 30h24M12 38h16" stroke="#D0D8EA" strokeWidth="2"
                        strokeLinecap="round"/>
                    </svg>
                    <div style={{ textAlign:"center" }}>
                      <div style={{ fontSize:13, fontWeight:600, color:"#374151", marginBottom:6 }}>
                        {doc?.title || "No document selected"}
                      </div>
                      {doc && (
                        <div style={{ fontSize:11, color:"#9CA3AF", lineHeight:1.6, marginBottom:14 }}>
                          {doc.desc || "Upload a PDF to preview it here"}
                        </div>
                      )}
                      {/* Upload button */}
                      <label style={{ cursor:"pointer" }}>
                        <input type="file" accept=".pdf,.doc,.docx" style={{ display:"none" }}
                          onChange={e => {
                            const file = e.target.files[0];
                            if (!file) return;
                            const dk = selectedDocNode;
                            if (nodeFiles[dk]?.objectUrl) URL.revokeObjectURL(nodeFiles[dk].objectUrl);
                            const objectUrl = URL.createObjectURL(file);
                            setNodeFiles(p => ({ ...p, [dk]: { filename:file.name, objectUrl, size:file.size } }));
                            setOutlinePreview({ url:objectUrl, filename:file.name,
                              section:doc?.section, title:doc?.title });
                            e.target.value = "";
                          }}
                        />
                        <div style={{ padding:"9px 20px", background:"#1A3D6B", color:"#fff",
              ...
              {/* ── ICH requirement info at bottom ── */}
              {doc?.desc && (
