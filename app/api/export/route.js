...
export async function GET(request) {
  ...
  try {
    ...
    // Get all nodes
    const { data: nodes } = await db.from('dossier_nodes').select('*').eq('dossier_id', dossier_id).order('section');
    ...
    const rootFolder = `${authority}-${product}-seq0000`;
...
Generated  : ${new Date().toISOString().split('T')[0]} by RAISA
Uploaded   : ${uploadedCount} of ${(nodes||[]).length} sections
