...
export const THEMES = {
  classic: {
    ...
    // The 4 Dashboard stat-card gradients — Classic keeps the exact original
    // AdminLTE-style hues, pixel for pixel.
    statGradients: [
    ...
    statTextColors: ['#fff', '#fff', '#fff', '#fff'],
  },
  volt: {
    name: 'volt',
    label: 'Volt',
    ...
    // 3 black "spotlight" cards + 1 bold lime card — the exact technique
    // seen in the Lumin reference (mostly-dark cards, one bright accent card
    // for the most important/positive number). Approved = the lime one,
    // since it's the success/complete end-state.
    statGradients: [
      'linear-gradient(135deg, #1a1a1a, #0A0A0A)',
      'linear-gradient(135deg, #1a1a1a, #0A0A0A)',
      'linear-gradient(135deg, #1a1a1a, #0A0A0A)',
      'linear-gradient(135deg, #D7FE03, #83AF3B)',
    ],
    statTextColors: ['#fff', '#fff', '#fff', '#0A0A0A'],
  },
};
